app.controller('clientPortofolioController', function($scope, $http, $ngBootbox, $location, CRUDService, NotificationService, constants) {
	$scope.dto = {};
	$scope.filtre = {};
	$scope.clientPortofolioTable = null;
	...
	$scope.baseUrl = "/client/portofolio/rest/";
	$scope.prospectBaseUrl = "/client/prospect/rest/";
	...
	$scope.init = function() {
		$scope.mode="read";
		$scope.addClientPortofolioForm = false;
		$scope.editClientPortofolioForm = false;
	...
	$scope.initListProspects = function() {
		$http.get(context+"/client/prospect/rest/getAllWithoutClientPortofolio").success(function(data, status) {   
			$scope.prospects = data;
		});
	};
	
	$scope.initListClients = function() {
		$http.get(context+"/client/client/rest/getAllWithoutClientPortofolio").success(function(data, status) {   
			$scope.clients = data;
		});
	};
	...
	$scope.addClientPortofolio = function() {
		...
		$scope.editClientPortofolioForm = false;
		$scope.addClientPortofolioForm = true;
		$scope.clientPortofolioSuccess = false;
		$scope.clientPortofolioTechnicalError = false;
		$scope.clientPortofolioRequiredError = false;
	...
	$scope.addProspect = function() {
		...
		$('#prospect').select2({placeholder: 'Choisir une option'}).select2('val', '');
	...
	editClientPortofolio = function() {
		CRUDService.edit($scope);
		$scope.addClientPortofolioForm = false;
		$scope.editClientPortofolioForm = true;
		$scope.clientPortofolioSuccess = false;
		$scope.clientPortofolioTechnicalError = false;
		$scope.clientPortofolioRequiredError = false;
		$('#clientPortofolioModal').modal("show");
	...
	$scope.loadProspect = function(id) {
		$http.get(context+$scope.prospectBaseUrl+"load/"+id).success(function(data, status) {
			$scope.dto.prospect = data;
		});
	};
	...
	$scope.searchClientPortofolio = function() {
		var searchKey = 'NAN';
		if(!angular.isUndefined($scope.dto.search.key) && $scope.dto.search.key != '') searchKey = $scope.dto.search.key;
		$scope.clientPortofolioTable.ajax.url('/client/portofolio/rest/search/'+searchKey).load();
	};
	
	$scope.saveClientPortofolio = function() {
		if($scope.clientPortofolioForm.$valid) {
	...
	$scope.saveProspect = function() {
		if($scope.prospectForm.$valid) {
			$scope.prospectId = ($("#prospect").val().split(':'))[1];
			$http.get(context+$scope.prospectBaseUrl+"load/"+$scope.prospectId).success(function(data, status) {
				$scope.dto.prospect = data;
				$scope.dto.prospect.clientPortofolioId = $scope.dto.id;
				$http.post(context+$scope.prospectBaseUrl+"save", angular.toJson($scope.dto.prospect)).success(function(data, status) {   
					$scope.refreshList();
					$scope.clientTechnicalError = false;
					$scope.clientRequiredError = false;
	...
	$scope.saveClient = function() {
		if($scope.clientForm.$valid) {
			...
			$http.get(context+$scope.clientBaseUrl+"load/"+$scope.clientId).success(function(data, status) {
				$scope.dto.client = data;
				$scope.dto.client.clientPortofolioId = $scope.dto.id;
	...
	$scope.saveClient = function(name) {
		...
		$http.post(context+$scope.clientBaseUrl+"saveClientPortofolio/"+$scope.dto.id, angular.toJson($scope.clientTabId)).success(function(data, status) {
			$scope.refreshList();
			$scope.clientTechnicalError = false;
			$scope.clientRequiredError = false;
			$scope.clientSuccess = true;
	...
	$scope.refreshList = function() {
		$scope.clientPortofolioTable.ajax.reload();
		$scope.dto = {};
	};

	removeClientPortofolio = function(id) {
		$scope.dto = {};
		if(confirm("Voulez-vous vraiment supprimer cet enregistrement ?") == true) {
			$scope.confirmDeleteClientPortofolio(id);
		};
	};

	$scope.confirmDeleteClientPortofolio = function(id) {
	...
	$scope.confirmDeleteClient = function(id) {
		$http.get(context+$scope.clientBaseUrl+"load/"+id).success(function(data, status) {
			$scope.dto.client = data;
			$scope.dto.client.clientPortofolioId = null;
			$http.post(context+$scope.clientBaseUrl+"save", angular.toJson($scope.dto.client)).success(function(data, status) {
				$scope.dto = {};
...
app.controller('clientPortofolioTableController', function($scope,$http) {
	$scope.$watch('$parent.dto.id', function(newValue, oldValue) {
		if(!angular.isUndefined(newValue) && newValue != null) {
			$scope.$parent.clientTable.ajax.url('/client/client/rest/listByClientPortofolio/'+newValue).load();
		}
		else {
			$scope.$parent.clientTable.ajax.url('/client/client/rest/listByClientPortofolio/-1').load();
		}
	});
	...
	$scope.init = function() {
		var columns = [ 
			...
			{mDataProp: 'id', "mRender": function(data, type, full) {
				return '<div class="form-check"><input class="form-check-input clientPortofolioId" type="checkbox" value="'+full.id+'" onclick="check(\'clientPortofolioId\')" /></div>';
			}},
			{mDataProp: 'name'},
			{mDataProp: 'description'},
			{mDataProp: 'clientNum'},
			{mDataProp: 'id', "mRender": function(data, type, full) {
				result = '<a href="#" onclick="addClient('+full.id+');" class="edit-item-btn"><i style="font-size: 17px" class="ri-user-add-fill align-bottom me-2 text-success"></i></a> <a href="#" onclick="editClientPortofolio();" class="edit-item-btn"><i style="font-size: 17px" class="ri-pencil-fill align-bottom me-2"></i></a> <a href="#" onclick="removeClientPortofolio('+full.id+');" class="remove-item-btn"><i style="font-size: 17px" class="ri-delete-bin-fill align-bottom me-2 text-danger"></i></a>';
				return result;
			}}
		];
		$scope.$parent.clientPortofolioTable = TableManager.init("clientPortofolioTable", $scope.$parent.baseUrl+"list", columns);
		
		$scope.$parent.clientPortofolioTable.on('select', function (e, dt, type, indexes) {
			var rowData = 	$scope.$parent.clientPortofolioTable.rows(indexes).data().toArray();
			var id = null;
			if(rowData.length > 0) {
...
app.controller('clientWithoutPortofolioTableController', function($scope,$http) {
	$scope.init = function() {
		...
		$scope.$parent.clientWithoutPortofolioTable = TableManager.init("clientWithoutPortofolioTable", $scope.$parent.clientBaseUrl+"listWithoutClientPortofolio", columns);
	};
});
...
app.controller('prospectTableController', function($scope,$http) {
	$scope.init = function() {
		var columns = [ 
			...
			{mDataProp: 'codeProspect', "mRender": function(data, type, full) {
				return '<a href="#" class="amount-link">'+full.codeProspect+'</a>';
			}},
			{mDataProp: 'iceNum'},
			{mDataProp: 'companyName', "mRender": function(data, type, full) {
				return '<a href="#" class="amount-link">'+full.companyName+'</a>';
			}},
			{mDataProp: 'countryName'},
			{mDataProp: 'cityName'},
			{mDataProp: 'id', "mRender": function(data, type, full) {
				result = '<a href="#" onclick="removeClient('+full.id+');" class="btn btn-fw danger" style="min-width: 1rem; padding-left: 8px; padding-right: 8px;  padding-top: 3px; padding-bottom: 3px;"><i class="fa fa-times"></i></a>';
				return result;
			}}
		];
		$scope.$parent.prospectTable = TableManager.init("prospectTable", $scope.$parent.prospectBaseUrl+"listByClientPortofolio/-1", columns);
...
app.controller('clientTableController', function($scope,$http) {
	$scope.init = function() {
		...
		$scope.$parent.clientTable = TableManager.init("clientTable", $scope.$parent.clientBaseUrl+"listByClientPortofolio/-1", columns);

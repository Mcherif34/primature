app.controller('courrierController', function($scope, $http, $ngBootbox, $location, CRUDService, NotificationService, constants) {
	...
	$scope.baseUrl = "/courrier/invoice/rest/";
	...
	$scope.advancedSearchInvoice = function() {
		var searchRefArriveeBoc = 'NAN'; var searchFactureDateStart = 'NAN'; var searchFactureDateEnd = 'NAN'; var searchNumFacture = 'NAN'; var searchReceptionDateStart = 'NAN'; var searchReceptionDateEnd = 'NAN'; var searchRefExpediteur = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchExpediteur = 'NAN'; var searchMontantFacture = 'NAN'; var searchStatus = 'NAN'; var searchAttestationRegularite = 'NAN'; var searchDecompteDateStart = 'NAN'; var searchDecompteDateEnd = 'NAN'; var searchNumDecompte = 'NAN'; var searchDevise = 'NAN'; var searchElementFacturation = 'NAN'; var searchNumBcContrat = 'NAN'; var searchTypeCourrier = 'NAN'; var searchDelaiPaiement = 'NAN'; var searchFormeJuridique = 'NAN'; var searchValidationDateStart = 'NAN'; var searchValidationDateEnd = 'NAN';
		...
		if(!angular.isUndefined($scope.dto.search.factureDateStart) && $scope.dto.search.factureDateStart != '') {
			searchFactureDateStart = $scope.dto.search.factureDateStart.toLocaleString("default", { day: "2-digit" }) + "-" + $scope.dto.search.factureDateStart.toLocaleString("default", { month: "2-digit" }) + "-" + $scope.dto.search.factureDateStart.toLocaleString("default", { year: "numeric" });
		}
		if(!angular.isUndefined($scope.dto.search.factureDateEnd) && $scope.dto.search.factureDateEnd != '') {
			searchFactureDateEnd = $scope.dto.search.factureDateEnd.toLocaleString("default", { day: "2-digit" }) + "-" + $scope.dto.search.factureDateEnd.toLocaleString("default", { month: "2-digit" }) + "-" + $scope.dto.search.factureDateEnd.toLocaleString("default", { year: "numeric" });
		}
		if(!angular.isUndefined($scope.dto.search.numFacture) && $scope.dto.search.numFacture != '') searchNumFacture = $scope.dto.search.numFacture.replaceAll("/", "@");
		...
		if(!angular.isUndefined($scope.dto.search.montantFacture) && $scope.dto.search.montantFacture != '') searchMontantFacture = $scope.dto.search.montantFacture;
		if(!angular.isUndefined($scope.dto.search.status) && $scope.dto.search.status != '') searchStatus = $scope.dto.search.status;
		if(!angular.isUndefined($scope.dto.search.attestationRegularite) && $scope.dto.search.attestationRegularite != '') searchAttestationRegularite = $scope.dto.search.attestationRegularite;
		if(!angular.isUndefined($scope.dto.search.decompteDateStart) && $scope.dto.search.decompteDateStart != '') {
			searchDecompteDateStart = $scope.dto.search.decompteDateStart.toLocaleString("default", { day: "2-digit" }) + "-" + $scope.dto.search.decompteDateStart.toLocaleString("default", { month: "2-digit" }) + "-" + $scope.dto.search.decompteDateStart.toLocaleString("default", { year: "numeric" });
		}
		if(!angular.isUndefined($scope.dto.search.decompteDateEnd) && $scope.dto.search.decompteDateEnd != '') {
			searchDecompteDateEnd = $scope.dto.search.decompteDateEnd.toLocaleString("default", { day: "2-digit" }) + "-" + $scope.dto.search.decompteDateEnd.toLocaleString("default", { month: "2-digit" }) + "-" + $scope.dto.search.decompteDateEnd.toLocaleString("default", { year: "numeric" });
		}
		if(!angular.isUndefined($scope.dto.search.numDecompte) && $scope.dto.search.numDecompte != '') searchNumDecompte = $scope.dto.search.numDecompte.replaceAll("/", "-");
		if(!angular.isUndefined($scope.dto.search.devise) && $scope.dto.search.devise != '') searchDevise = $scope.dto.search.devise;
		if(!angular.isUndefined($scope.dto.search.elementFacturation) && $scope.dto.search.elementFacturation != '') searchElementFacturation = $scope.dto.search.elementFacturation;
		if(!angular.isUndefined($scope.dto.search.numBcContrat) && $scope.dto.search.numBcContrat != '') searchNumBcContrat = $scope.dto.search.numBcContrat.replaceAll("/", "-");
		...
		if(!angular.isUndefined($scope.dto.search.delaiPaiement) && $scope.dto.search.delaiPaiement != '') searchDelaiPaiement = $scope.dto.search.delaiPaiement;
		if(!angular.isUndefined($scope.dto.search.formeJuridique) && $scope.dto.search.formeJuridique != '') searchFormeJuridique = $scope.dto.search.formeJuridique;
		if(!angular.isUndefined($scope.dto.search.validationDateStart) && $scope.dto.search.validationDateStart != '') {
			searchValidationDateStart = $scope.dto.search.validationDateStart.toLocaleString("default", { day: "2-digit" }) + "-" + $scope.dto.search.validationDateStart.toLocaleString("default", { month: "2-digit" }) + "-" + $scope.dto.search.validationDateStart.toLocaleString("default", { year: "numeric" });
		}
		if(!angular.isUndefined($scope.dto.search.validationDateEnd) && $scope.dto.search.validationDateEnd != '') {
			searchValidationDateEnd = $scope.dto.search.validationDateEnd.toLocaleString("default", { day: "2-digit" }) + "-" + $scope.dto.search.validationDateEnd.toLocaleString("default", { month: "2-digit" }) + "-" + $scope.dto.search.validationDateEnd.toLocaleString("default", { year: "numeric" });
		}
		
		$scope.courrierTable.ajax.url($scope.baseUrl+'search/'+searchRefArriveeBoc+'/'+searchFactureDateStart+'/'+searchFactureDateEnd+'/'+searchNumFacture+'/'+searchReceptionDateStart+'/'+searchReceptionDateEnd+'/'+searchRefExpediteur+'/'+searchEnregistrementDateStart+'/'+searchEnregistrementDateEnd+'/'+searchExpediteur+'/'+searchMontantFacture+'/'+searchStatus+'/'+searchAttestationRegularite+'/'+searchDecompteDateStart+'/'+searchDecompteDateEnd+'/'+searchNumDecompte+'/'+searchDevise+'/'+searchElementFacturation+'/'+searchNumBcContrat+'/'+searchTypeCourrier+'/'+searchDelaiPaiement+'/'+searchFormeJuridique+'/'+searchValidationDateStart+'/'+searchValidationDateEnd).load();
	};
...
;
	...
	$scope.exportSheets = function(name) {
		...
		$scope.sheetTab = [];
		var title = ["REFERENCE COURRIER", "DATE ENREGISTREMENT", "DATE RECEPTION", "DATE FACTURE", "Nº FACTURE", "EXPEDITEUR", "MONTANT FACTURE", "ATTESTATION DE REGULARITE", "ETAPE EN COURS", "STATUT"];
		...
		for(var i = 0, l = inputs.length; i < l; ++i) {
			if(inputs[i].checked) {
				$http.get(context+$scope.baseUrl+"loadByReference/"+inputs[i].value.replaceAll("/", "-")).success(function(data, status) {
					...
					sheet[3] = data.dateFacture;
					sheet[4] = data.numFacture;
					sheet[5] = data.expediteur;
					sheet[6] = data.montantFacture;
					sheet[7] = data.attestationRegularite;
					sheet[8] = data.taskTitle;
					var status = "";
	...
	$scope.downloadFile = function() {
		exportExcelFile($scope.sheetTab, 'CourrierFactures.xlsx', 'CourrierFactures');
	}
...
app.controller('courrierTableController', function($scope,$http) {
	...
	$scope.init = function() {
		var columns = [ 
			...
			{mDataProp: 'attestationRegularite'},
			{mDataProp: 'taskTitle'},

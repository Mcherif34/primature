app.controller('promiseController', function($scope, $http, $ngBootbox, $location, CRUDService, NotificationService, constants) {
	$scope.dto = {};
	$scope.filtre = {};
	$scope.promiseCurrentTable = null;
	$scope.promiseCloseTable = null;
	$scope.promiseStageTable = null;
	...
	$scope.promiseStageRequiredError = false;
	$scope.promiseStageTechnicalError = false;
	$scope.promiseStageSuccess = false;
	$scope.promiseStageDateRequiredError = false;
	$scope.promiseStageDateTechnicalError = false;
	$scope.promiseStageDateSuccess = false;
	$scope.notifications = null;
	$scope.notificationsCount = 0;
	
	$scope.baseUrl = "/reminder/promise/rest/";
	$scope.promiseStageBaseUrl = "/reminder/promiseStage/rest/";
	...
	$scope.addPromiseStage = function() {
		$scope.dto.promiseStage = {};
		$scope.editPromiseStageForm = false;
		$scope.addPromiseStageForm = true;
		$scope.promiseStageSuccess = false;
		$scope.promiseStageTechnicalError = false;
		$scope.promiseStageRequiredError = false;
		$('#promiseStageModal').modal("show");
	};
	
	$scope.addPromiseInvoice = function() {
		$scope.dto.promise = {};
		if(!angular.isUndefined($scope.invoiceDetail)) {
			...
			$http.get(context+"/credit/invoice/rest/load/"+$scope.invoiceDetail.id).success(function(data, status) {
				...
				$scope.promiseTechnicalError = false;
				$scope.promiseRequiredError = false;
				$('#promiseInvoiceModal').modal("show");
			});
		}
	};
	...
	editPromiseStage = function(id) {
		$http.get(context+$scope.promiseStageBaseUrl+"load/"+id).success(function(data, status) { 
			$scope.promiseStage = data;
			$scope.dto.promiseStage = $scope.promiseStage;
			CRUDService.edit($scope);
			$scope.addPromiseStageForm = false;
			$scope.editPromiseStageForm = true;
			$scope.promiseStageSuccess = false;
			$scope.promiseStageTechnicalError = false;
			$scope.promiseStageRequiredError = false;
			$('#promiseStageModal').modal("show");
	...
	$scope.editPromiseStageDate = function() {
		CRUDService.edit($scope);
		$scope.promiseStageDateSuccess = false;
		$scope.promiseStageDateTechnicalError = false;
		$scope.promiseStageDateRequiredError = false;
		$('#promiseStageDateModal').modal("show");
	};
	...
	$scope.loadPromiseStage = function(id) {
		$http.get(context+$scope.promiseStageBaseUrl+"load/"+id).success(function(data, status) { 
			$scope.promiseStage = data;
			$scope.dto.promiseStage = $scope.promiseStage;
		});
		...
		if($scope.promiseForm.$valid) {
			...
			if($scope.dto.code == null) {
				...
				$http.get(context+"/reminder/promise/rest/getAll").success(function(data, status) {   
					...
					$scope.dto.code = "PR" + currentDate.getFullYear() + (currentDate.getMonth()+1).toString().padStart(2, '0') + seqNum.toString().padStart(3, '0');
	...
	$scope.savePromiseInvoice = function() {
		if($scope.promiseForm.$valid) {
			...
			if($scope.dto.promise.code == null) {
				...
				$http.get(context+"/reminder/promise/rest/getAll").success(function(data, status) {   
					...
					$http.post(context+$scope.baseUrl+"save", angular.toJson($scope.dto.promise)).success(function(data, status) {
						$scope.promiseInvoiceTable.ajax.url('/reminder/promise/rest/listByInvoice/'+data.invoiceId).load();
						$scope.promiseCurrentTable.ajax.reload();
						...
						$scope.promiseRequiredError = false;
						$scope.promiseSuccess = true;
						$('#promiseInvoiceModal').modal("hide");
					}).error(function(data, status, headers, config) {
						$scope.promiseTechnicalError = true;
					});
				});
	...
	$scope.closePromise = function() {
		if(confirm("Voulez-vous vraiment clôturer cette promesse de règlement ?") == true) {
			$scope.dto.promiseStatusId = 2;
			CRUDService.save($scope,$scope.dto).success(function(data, status) {   
				CRUDService.setEntityLoaded($scope,data);
				$scope.refreshList();
			}).error(function(data, status, headers, config) {
	...
	$scope.savePromiseStage = function() {
		if($scope.promiseStageForm.$valid) {
			$scope.dto.promiseStage.promiseId = $scope.dto.id;
			$http.post( context+$scope.promiseStageBaseUrl+"save", angular.toJson($scope.dto.promiseStage)).success(function(data, status) {   
				$scope.dto = {};
				$scope.load($scope.selected.id);
				$scope.promiseStageTechnicalError = false;
				$scope.promiseStageRequiredError = false;
				$scope.promiseStageSuccess = true;
				$('#promiseStageModal').modal("hide");
			}).error(function(data, status, headers, config) {
				$scope.promiseStageTechnicalError = true;
			});
		} else {
			$scope.promiseStageRequiredError = true;
		}
	};
	
	$scope.savePromiseStageDate = function() {
		if($scope.promiseStageDateForm.$valid) {
			$http.post( context+$scope.promiseStageBaseUrl+"save", angular.toJson($scope.dto.promiseStage)).success(function(data, status) {   
				$scope.dto = {};
				$scope.load($scope.selected.id);
				$scope.promiseStageDateTechnicalError = false;
				$scope.promiseStageDateRequiredError = false;
				$scope.promiseStageDateSuccess = true;
				$('#promiseStageDateModal').modal("hide");
			}).error(function(data, status, headers, config) {
				$scope.promiseStageDateTechnicalError = true;
			});
		} else {
			$scope.promiseStageDateRequiredError = true;
		}
	};
	...
	removePromise = function(id) {
		...
		if(confirm("Voulez-vous vraiment supprimer cet enregistrement ?") == true) {
			$scope.confirmDeletePromise(id);
		};
	};

	$scope.confirmDeletePromise = function(id) {
	...
	removePromiseStage = function(id) {
		$scope.dto = {};
		if(confirm("Voulez-vous vraiment supprimer cet enregistrement ?") == true) {
			$scope.confirmDeletePromiseStage(id);
		};
	};

	$scope.confirmDeletePromiseStage = function(id) {
		$http.post(context+$scope.promiseStageBaseUrl+"delete/"+id).success(function(data, status) { 
			$scope.dto = {};
			$scope.load($scope.selected.id);
		}).error(function(data, status, headers, config) {
			console.log("error");
...
app.controller('promiseCurrentTableController', function($scope,$http) {
	$scope.$watch('$parent.dto.id', function(newValue, oldValue) {
		if(!angular.isUndefined(newValue) && newValue != null) {
			$scope.$parent.promiseStageTable.ajax.url('/reminder/promiseStage/rest/list/'+newValue).load();
		}
		else {
			$scope.$parent.promiseStageTable.ajax.url('/reminder/promiseStage/rest/list/-1').load();
		}
	});
	...
	$scope.init = function() {
		var columns = [ 
			...
			{mDataProp: 'id', "mRender": function(data, type, full) {
				result = '<a href="#" onclick="editPromise('+full.id+');" class="btn btn-fw orange" style="min-width: 1rem; padding-left: 8px; padding-right: 8px; padding-top: 3px; padding-bottom: 3px;"><i class="fa fa-pencil"></i></a> <a href="#" onclick="removePromise('+full.id+');" class="btn btn-fw danger" style="min-width: 1rem; padding-left: 8px; padding-right: 8px; padding-top: 3px; padding-bottom: 3px;"><i class="fa fa-trash"></i></a>';
				return result;
			}}
		];
		$scope.$parent.promiseCurrentTable = TableManager.init("promiseCurrentTable", $scope.$parent.baseUrl+"list/current", columns);
		
		$scope.$parent.promiseCurrentTable.on('select', function (e, dt, type, indexes) {
			var rowData = $scope.$parent.promiseCurrentTable.rows(indexes).data().toArray();
			var id = null;
			if(rowData.length > 0) {
...
app.controller('promiseCloseTableController', function($scope,$http) {
	$scope.$watch('$parent.dto.id', function(newValue, oldValue) {
		if(!angular.isUndefined(newValue) && newValue != null) {
			$scope.$parent.promiseStageTable.ajax.url('/reminder/promiseStage/rest/list/'+newValue).load();
		}
	...
	$scope.init = function() {
		var columns = [ 
			...
			{mDataProp: 'clientCode', "mRender": function(data, type, full) {
				return '<a href="#" class="amount-link">'+full.clientCode+'</a>';
			}},
			{mDataProp: 'clientCompanyName', "mRender": function(data, type, full) {
				return '<a href="#" class="amount-link">'+full.clientCompanyName+'</a>';
			}},
			{mDataProp: 'id', "mRender": function(data, type, full) {
				result = '<a href="#" onclick="editPromise('+full.id+');" class="btn btn-fw orange" style="min-width: 1rem; padding-left: 8px; padding-right: 8px; padding-top: 3px; padding-bottom: 3px;"><i class="fa fa-pencil"></i></a> <a href="#" onclick="removePromise('+full.id+');" class="btn btn-fw danger" style="min-width: 1rem; padding-left: 8px; padding-right: 8px; padding-top: 3px; padding-bottom: 3px;"><i class="fa fa-trash"></i></a>';
				return result;
		...
		$scope.$parent.promiseCloseTable = TableManager.init("promiseCloseTable", $scope.$parent.baseUrl+"list/close", columns);
		
		$scope.$parent.promiseCloseTable.on('select', function (e, dt, type, indexes) {
			var rowData = $scope.$parent.promiseCloseTable.rows(indexes).data().toArray();
			var id = null;
			if(rowData.length > 0) {
				$scope.$parent.selected = rowData[0];
				id = rowData[0].id;
...
app.controller('promiseStageTableController', function($scope,$http) {
	$scope.init = function() {
		var columns = [
			{mDataProp: 'id', "visible": false},
			{mDataProp: 'date'},
			...
			{mDataProp: 'id', "mRender": function(data, type, full) {
				result = '<a href="#" onclick="editPromiseStage('+full.id+');" class="btn btn-fw info" style="min-width: 1rem; padding-left: 8px; padding-right: 8px; padding-top: 3px; padding-bottom: 3px;"><i class="fa fa-pencil"></i></a> <a href="#" onclick="removePromiseStage('+full.id+');" class="btn btn-fw danger" style="min-width: 1rem; padding-left: 8px; padding-right: 8px; padding-top: 3px; padding-bottom: 3px;"><i class="fa fa-trash"></i></a>';
				return result;
			}}
		];
		$scope.$parent.promiseStageTable = TableManager.init("promiseStageTable", $scope.$parent.promiseStageBaseUrl+"list/-1", columns);
		
		$scope.$parent.promiseStageTable.on('select', function (e, dt, type, indexes) {
			var rowData = $scope.$parent.promiseStageTable.rows(indexes).data().toArray();
			var id = null;
			if(rowData.length > 0) {
				id = rowData[0].id;
				$scope.$parent.loadPromiseStage(id);
			}
		});
	};
});
...
app.controller('promiseInvoiceTableController', function($scope,$http) {
	$scope.init = function() {
		...
		$scope.$parent.promiseInvoiceTable = TableManager.init("promiseInvoiceTable", $scope.$parent.baseUrl+"listByInvoice/-1", columns);
	};
});

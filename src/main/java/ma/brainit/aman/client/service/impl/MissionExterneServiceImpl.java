...
import ma.brainit.aman.client.dao.MissionExterneDao;
...
import ma.brainit.aman.client.dto.converters.MissionExterneDTOConverter;
...
public class MissionExterneServiceImpl implements MissionExterneService {
	...
	@Value("${opentext.courrierMissionExterneID}")
	private Long opentextCourrierMissionExterneID;
	
	private GedService ged = new GedService();
	
	@Autowired
	private MissionExterneDTOConverter MissionExterneDTOConverter;

	@Autowired
	private BasePaginatorDao<MissionExterne, Long> paginatorDao;

	@Autowired
	private MissionExterneDao MissionExterneDao;
	...
	public String getPage(Integer page, Integer limit, String sort, String direction, String search) {
		...
		searchParams  = MissionExterneDTOConverter.convertSearchParamToEntity(search);
		this.paginatorDao.setEntityClass(MissionExterne.class);
		List<MissionExterne> list = paginatorDao.getPaginator(page, limit,sort,direction,searchParams, condition.toString());
		Long totalCount = paginatorDao.count(searchParams, condition.toString());
		List<MissionExterneDTO> dtos = MissionExterneDTOConverter.convertFromDataBeanList(list);
		return Util.toJson(new BaseTable<MissionExterneDTO>(dtos,totalCount));
	}
	...
	@Override
	@Transactional
	public String save(MissionExterneDTO dto) {
		SecUtilisateurDTO user = secUtilisateurService.getCurrentUser();
		...
		String result = ged.initiateMissionExterne(opentextUrl, opentextCourrierMissionExterneID, dto, user.getLogin(), reference, authToken);
		if(result == "FAILED") {
			return result;
		}
	...
	@Override
	@Transactional
	public MissionExterneDTO load(Long id) {
		MissionExterne entity = MissionExterneDao.findOne(id);
		MissionExterneDTO dto = MissionExterneDTOConverter.convertFromDataBean(entity);
	...
	@Override
	@Transactional
	public MissionExterneDTO loadByReference(String reference) {
		MissionExterneDTO dto = MissionExterneDTOConverter.convertFromDataBean(MissionExterneDao.getByReference(reference.replaceAll("-", "/")));
		return dto;
	}
	
	@Override
	@Transactional
	public MissionExterneDTO loadByVolumeId(Long volumeId) {
		MissionExterneDTO dto = MissionExterneDTOConverter.convertFromDataBean(MissionExterneDao.getByVolumeId(volumeId));
		return dto;
	}
	...
	public List<DocumentDTO> loadDocumentsByReference(String reference) {
		MissionExterne entity = MissionExterneDao.getByReference(reference.replaceAll("-", "/"));
		List<DocumentDTO> documents = new ArrayList<DocumentDTO>();
		String authToken = ged.authentication(opentextUrl, opentextLogin, opentextPassword);
		try {
			if(entity != null && entity.getwSubWork() != null) {
	...
	@Override
	@Transactional
	public String send(MissionExterneDTO dto) {
		SecUtilisateurDTO user = secUtilisateurService.getCurrentUser();
		String authToken = ged.authentication(opentextUrl, opentextLogin, opentextPassword);
		String result = "";
//		result = ged.getActivitiesList(opentextUrl, dto, authToken);
		if(dto.getTaskTitle().equals("Réception du courrier")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Réception du courrier", "Motif Rejet Courrier", 1, authToken);
			else
				result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Réception du courrier", null, 1, authToken);
		}
		if(dto.getTaskTitle().equals("Annotation PM")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Annotation PM", "Motif Rejet Annotation PM", 2, authToken);
			else
				result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Annotation PM", null, 2, authToken);
		}
		if(dto.getTaskTitle().equals("Préparation SOM")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Préparation SOM", "Motif Rejet Préparation SOM", 3, authToken);
			else
				result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Préparation SOM", null, 3, authToken);
		}
		if(dto.getTaskTitle().equals("Visa Conseiller")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Visa Conseiller", "Motif Rejet Visa Conseiller", 4, authToken);
			else
				result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Visa Conseiller", null, 4, authToken);
		}
		if(dto.getTaskTitle().equals("Visa SG")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Visa SG", "Motif Rejet Visa SG", 5, authToken);
			else
				result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Visa SG", null, 5, authToken);
		}
		if(dto.getTaskTitle().equals("Signature PM")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Signature PM", "Motif Rejet Signature PM", 6, authToken);
			else
				result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Signature PM", null, 6, authToken);
		}
		if(dto.getTaskTitle().equals("Transfert SG")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Transfert SG", "Motif Rejet Transfert SG", 7, authToken);
			else
				result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Transfert SG", null, 7, authToken);
		}
		if(dto.getTaskTitle().equals("Annotation SG")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Annotation SG", "Motif Rejet Annotation SG", 22, authToken);
			else
				result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Annotation SG", null, 22, authToken);
		}
		if(dto.getTaskTitle().equals("[Rejet] Initiation"))
			result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Rejet Initiation", null, 16, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Préparation SOM"))
			result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Rejet Préparation SOM", null, 17, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Visa Conseiller"))
			result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Rejet Visa Conseiller", null, 18, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Visa SG"))
			result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Rejet Visa SG", null, 19, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Signature PM"))
			result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Rejet Signature PM", null, 20, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Réception du courrier"))
			result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Rejet Réception du courrier", null, 21, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Annotation PM"))
			result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Rejet Annotation PM", null, 24, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Annotation SG"))
			result = ged.sendMissionExterne(opentextUrl, dto, user.getLogin(), "Rejet Annotation SG", null, 25, authToken);
	...
	@Override
	@Transactional
	public List<MissionExterneDTO> getAll() {
		List<MissionExterne> list = MissionExterneDao.findAll();
		List<MissionExterneDTO> dtos = MissionExterneDTOConverter.convertFromDataBeanList(list);
	...
	public int getCurrentCountByEntity(String term) {
		return MissionExterneDao.getCurrentCountByEntity(term);
	}
	
	@Override
	@Transactional
	public int getCurrentCount() {
		return MissionExterneDao.getCurrentCount();
	}
	
	@Override
	@Transactional
	public int getCompletedCount() {
		return MissionExterneDao.getCompletedCount();
	}
	
	@Override
	@Transactional
	public int getRejectedCount() {
		return MissionExterneDao.getRejectedCount();
	}
	...
	public int getCurrentCountByProfile() {
		...
		return MissionExterneDao.getCurrentCountByProfile(profile.getPerformerName());
	}
	
	@Override
	@Transactional
	public List<Object> getCurrentCountByYear(int year) {
		return MissionExterneDao.getCurrentCountByYear(year);
	}
	
	@Override
	@Transactional
	public List<Object> getCompletedCountByYear(int year) {
		return MissionExterneDao.getCompletedCountByYear(year);
	}
	
	@Override
	@Transactional
	public int getCurrentNotOverdueClassiqueCount() {
		return MissionExterneDao.getCurrentNotOverdueClassiqueCount();
	}
	
	@Override
	@Transactional
	public int getCurrentOverdueClassiqueCount() {
		return MissionExterneDao.getCurrentOverdueClassiqueCount();
	}
	
	@Override
	@Transactional
	public int getCompletedClassiqueCount() {
		return MissionExterneDao.getCompletedClassiqueCount();
	}

...
import ma.brainit.aman.client.dao.CorrespondanceSgDao;
...
import ma.brainit.aman.client.dto.converters.CorrespondanceSgDTOConverter;
...
@Service
@Transactional
public class CorrespondanceSgServiceImpl implements CorrespondanceSgService {
	...
	@Value("${opentext.courrierCorrespondanceSgID}")
	private Long opentextCourrierCorrespondanceSgID;
	
	private GedService ged = new GedService();
	
	@Autowired
	private CorrespondanceSgDTOConverter CorrespondanceDTOConverter;

	@Autowired
	private BasePaginatorDao<CorrespondanceSg, Long> paginatorDao;

	@Autowired
	private CorrespondanceSgDao CorrespondanceDao;
	...
	public String getPage(Integer page, Integer limit, String sort, String direction, String search) {
		...
		this.paginatorDao.setEntityClass(CorrespondanceSg.class);
		List<CorrespondanceSg> list = paginatorDao.getPaginator(page, limit,sort,direction,searchParams, condition.toString());
		Long totalCount = paginatorDao.count(searchParams, condition.toString());
		List<CorrespondanceSgDTO> dtos = CorrespondanceDTOConverter.convertFromDataBeanList(list);
//		for(CorrespondanceDTO CorrespondanceDTO : dtos) {
//			WSubWorkTask wSubWorkTask = wSubWorkTaskDao.getByWWork(CorrespondanceDTO.getwSubWorkId());
//			if(wSubWorkTask != null) {
//				CorrespondanceDTO.setTaskTitle(wSubWorkTask.getSubWorkTaskTitle());
//				CorrespondanceDTO.setPerformer(wSubWorkTask.getPerformer().getName());
//			}
//		}
		return Util.toJson(new BaseTable<CorrespondanceSgDTO>(dtos,totalCount));
	}
...
//		}
	...
	@Override
	@Transactional
	public String save(CorrespondanceSgDTO dto) {
		SecUtilisateurDTO user = secUtilisateurService.getCurrentUser();
		...
		String result = ged.initiateCorrespondanceSg(opentextUrl, opentextCourrierCorrespondanceSgID, dto, user.getLogin(), reference, authToken);
		if(result == "FAILED") {
			return result;
		}
	...
	@Override
	@Transactional
	public CorrespondanceSgDTO load(Long id) {
		CorrespondanceSg entity = CorrespondanceDao.findOne(id);
		CorrespondanceSgDTO dto = CorrespondanceDTOConverter.convertFromDataBean(entity);
	...
	@Override
	@Transactional
	public CorrespondanceSgDTO loadByReference(String reference) {
		CorrespondanceSgDTO dto = CorrespondanceDTOConverter.convertFromDataBean(CorrespondanceDao.getByReference(reference.replaceAll("-", "/")));
		return dto;
	}
	
	@Override
	@Transactional
	public CorrespondanceSgDTO loadByVolumeId(Long volumeId) {
		CorrespondanceSgDTO dto = CorrespondanceDTOConverter.convertFromDataBean(CorrespondanceDao.getByVolumeId(volumeId));
		return dto;
	}
	...
	public List<DocumentDTO> loadDocumentsByReference(String reference) {
		CorrespondanceSg entity = CorrespondanceDao.getByReference(reference.replaceAll("-", "/"));
		List<DocumentDTO> documents = new ArrayList<DocumentDTO>();
		String authToken = ged.authentication(opentextUrl, opentextLogin, opentextPassword);
		try {
			if(entity != null && entity.getwSubWork() != null) {
	...
	@Override
	@Transactional
	public String send(CorrespondanceSgDTO dto) {
		SecUtilisateurDTO user = secUtilisateurService.getCurrentUser();
		String authToken = ged.authentication(opentextUrl, opentextLogin, opentextPassword);
		String result = "";
//		result = ged.getActivitiesList(opentextUrl, dto, authToken);
		if(dto.getTaskTitle().equals("Réception du courrier")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendCorrespondanceSg(opentextUrl, dto, user.getLogin(), "Initiation", "Motif Rejet Courrier", 3, authToken);
			else
				result = ged.sendCorrespondanceSg(opentextUrl, dto, user.getLogin(), "Initiation", null, 3, authToken);
		}
		if(dto.getTaskTitle().equals("Annotation PM")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendCorrespondanceSg(opentextUrl, dto, user.getLogin(), "Annotation PM", "Motif Rejet Annotation PM", 4, authToken);
			else
				result = ged.sendCorrespondanceSg(opentextUrl, dto, user.getLogin(), "Annotation PM", null, 4, authToken);
		}
		if(dto.getTaskTitle().equals("Annotation SG")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendCorrespondanceSg(opentextUrl, dto, user.getLogin(), "Annotation SG", "Motif Rejet Annotation SG", 5, authToken);
			else
				result = ged.sendCorrespondanceSg(opentextUrl, dto, user.getLogin(), "Annotation SG", null, 5, authToken);
		}
		if(dto.getTaskTitle().equals("Vérification Coordonnateur"))
			result = ged.sendCorrespondanceSg(opentextUrl, dto, user.getLogin(), "Vérification Coordonnateur", null, 6, authToken);
		if(dto.getTaskTitle().equals("Transfert SG")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendCorrespondanceSg(opentextUrl, dto, user.getLogin(), "Transfert SG", "Motif Rejet Transfert SG", 7, authToken);
			else
				result = ged.sendCorrespondanceSg(opentextUrl, dto, user.getLogin(), "Transfert SG", null, 7, authToken);
		}
		if(dto.getTaskTitle().equals("Traitement Conseiller")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendCorrespondanceSg(opentextUrl, dto, user.getLogin(), "Traitement Conseiller", "Motif Rejet Traitement Conseiller", 8, authToken);
			else
				result = ged.sendCorrespondanceSg(opentextUrl, dto, user.getLogin(), "Traitement Conseiller", null, 8, authToken);
		}
		if(dto.getTaskTitle().equals("Signature PM")) {
			result = ged.sendCorrespondanceSg(opentextUrl, dto, user.getLogin(), "Signature PM", null, 9, authToken);
		}
		if(dto.getTaskTitle().equals("Signature SG")) {
			result = ged.sendCorrespondanceSg(opentextUrl, dto, user.getLogin(), "Signature SG", null, 19, authToken);
		}
		if(dto.getTaskTitle().equals("Envoi du courrier PM"))
			result = ged.sendCorrespondanceSg(opentextUrl, dto, user.getLogin(), "Envoi du courrier", null, 10, authToken);
		if(dto.getTaskTitle().equals("Envoi du courrier SG"))
			result = ged.sendCorrespondanceSg(opentextUrl, dto, user.getLogin(), "Envoi du courrier", null, 20, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Initiation"))
			result = ged.sendCorrespondanceSg(opentextUrl, dto, user.getLogin(), "Rejet Initiation", null, 18, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Réception du courrier"))
			result = ged.sendCorrespondanceSg(opentextUrl, dto, user.getLogin(), "Rejet Réception du courrier", null, 11, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Annotation PM"))
			result = ged.sendCorrespondanceSg(opentextUrl, dto, user.getLogin(), "Rejet Annotation PM", null, 12, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Vérification Coordonnateur"))
			result = ged.sendCorrespondanceSg(opentextUrl, dto, user.getLogin(), "Rejet Vérification Coordonnateur", null, 15, authToken);
	...
	@Override
	@Transactional
	public List<CorrespondanceSgDTO> getAll() {
		List<CorrespondanceSg> list = CorrespondanceDao.findAll();
		List<CorrespondanceSgDTO> dtos = CorrespondanceDTOConverter.convertFromDataBeanList(list);

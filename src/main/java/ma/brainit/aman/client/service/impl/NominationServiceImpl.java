...
import ma.brainit.aman.client.dao.NominationDao;
...
import ma.brainit.aman.client.dto.converters.NominationDTOConverter;
...
public class NominationServiceImpl implements NominationService {
	...
	@Value("${opentext.courrierNominationID}")
	private Long opentextCourrierNominationID;
	
	private GedService ged = new GedService();
	
	@Autowired
	private NominationDTOConverter NominationDTOConverter;

	@Autowired
	private BasePaginatorDao<Nomination, Long> paginatorDao;
	...
	public String getPage(Integer page, Integer limit, String sort, String direction, String search) {
		...
		searchParams  = NominationDTOConverter.convertSearchParamToEntity(search);
		this.paginatorDao.setEntityClass(Nomination.class);
		List<Nomination> list = paginatorDao.getPaginator(page, limit,sort,direction,searchParams, condition.toString());
		Long totalCount = paginatorDao.count(searchParams, condition.toString());
		List<NominationDTO> dtos = NominationDTOConverter.convertFromDataBeanList(list);
		return Util.toJson(new BaseTable<NominationDTO>(dtos,totalCount));
	}
	...
	@Override
	public String advancedSearch(Integer page, Integer limit, String sort, String direction, String search, String referenceCourrier, String referenceNomination, String enregistrementDateStart, String enregistrementDateEnd, String objet, String status) {
		SecUtilisateurDTO user = secUtilisateurService.getCurrentUser();
		SecProfileDTO group = secProfileService.load(user.getSecProfileId());
		...
		if(referenceNomination != null && !referenceNomination.equals("NAN")) {
			condition.append(" AND e.referenceNomination LIKE '%").append(referenceNomination.toUpperCase().replaceAll("-", "/")).append("%'");
		}
	...
	@Override
	@Transactional
	public String save(NominationDTO dto) {
		SecUtilisateurDTO user = secUtilisateurService.getCurrentUser();
		...
		String result = ged.initiateNomination(opentextUrl, opentextCourrierNominationID, dto, user.getLogin(), reference, authToken);
		if(result == "FAILED") {
			return result;
		}
	...
	@Override
	@Transactional
	public NominationDTO load(Long id) {
		Nomination entity = NominationDao.findOne(id);
		NominationDTO dto = NominationDTOConverter.convertFromDataBean(entity);
	...
	@Override
	@Transactional
	public NominationDTO loadByReference(String reference) {
		NominationDTO dto = NominationDTOConverter.convertFromDataBean(NominationDao.getByReference(reference.replaceAll("-", "/")));
		return dto;
	}
	
	@Override
	@Transactional
	public NominationDTO loadByVolumeId(Long volumeId) {
		NominationDTO dto = NominationDTOConverter.convertFromDataBean(NominationDao.getByVolumeId(volumeId));
		return dto;
	}
	...
	public List<DocumentDTO> loadDocumentsByReference(String reference) {
		Nomination entity = NominationDao.getByReference(reference.replaceAll("-", "/"));
		List<DocumentDTO> documents = new ArrayList<DocumentDTO>();
		String authToken = ged.authentication(opentextUrl, opentextLogin, opentextPassword);
		try {
			if(entity != null && entity.getwSubWork() != null) {
	...
	@Override
	@Transactional
	public String send(NominationDTO dto) {
		SecUtilisateurDTO user = secUtilisateurService.getCurrentUser();
		String authToken = ged.authentication(opentextUrl, opentextLogin, opentextPassword);
		String result = "";
//		result = ged.getActivitiesList(opentextUrl, dto, authToken);
		if(dto.getTaskTitle().equals("Codification SGG")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendNomination(opentextUrl, dto, user.getLogin(), "Codification SGG", "Motif Rejet Codification SGG", 1, authToken);
			else
				result = ged.sendNomination(opentextUrl, dto, user.getLogin(), "Codification SGG", null, 1, authToken);
		}
		if(dto.getTaskTitle().equals("[Rejet] Initiation"))
			result = ged.sendNomination(opentextUrl, dto, user.getLogin(), "Rejet Initiation", null, 4, authToken);
	...
	public List<NominationDTO> getAll() {
		List<Nomination> list = NominationDao.findAll();
		List<NominationDTO> dtos = NominationDTOConverter.convertFromDataBeanList(list);
	...
	public int getCurrentCountByEntity(String term) {
		return NominationDao.getCurrentCountByEntity(term);
	}
	
	@Override
	@Transactional
	public int getCurrentCount() {
		return NominationDao.getCurrentCount();
	}
	
	@Override
	@Transactional
	public int getCompletedCount() {
		return NominationDao.getCompletedCount();
	}
	
	@Override
	@Transactional
	public int getRejectedCount() {
		return NominationDao.getRejectedCount();
	}
	...
	public int getCurrentCountByProfile() {
		...
		return NominationDao.getCurrentCountByProfile(profile.getPerformerName());
	}
	
	@Override
	@Transactional
	public List<Object> getCurrentCountByYear(int year) {
		return NominationDao.getCurrentCountByYear(year);
	}
	
	@Override
	@Transactional
	public List<Object> getCompletedCountByYear(int year) {
		return NominationDao.getCompletedCountByYear(year);
	}
	
	@Override
	@Transactional
	public int getCurrentNotOverdueClassiqueCount() {
		return NominationDao.getCurrentNotOverdueClassiqueCount();
	}
	
	@Override
	@Transactional
	public int getCurrentOverdueClassiqueCount() {
		return NominationDao.getCurrentOverdueClassiqueCount();
	}
	
	@Override
	@Transactional
	public int getCompletedClassiqueCount() {
		return NominationDao.getCompletedClassiqueCount();
	}

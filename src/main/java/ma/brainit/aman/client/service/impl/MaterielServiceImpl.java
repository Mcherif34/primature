...
import ma.brainit.aman.client.dao.MaterielDao;
...
import ma.brainit.aman.client.dto.converters.MaterielDTOConverter;
...
public class MaterielServiceImpl implements MaterielService {
	...
	@Value("${opentext.courrierMaterielID}")
	private Long opentextCourrierMaterielID;
	
	private GedService ged = new GedService();
	
	@Autowired
	private MaterielDTOConverter MaterielDTOConverter;

	@Autowired
	private BasePaginatorDao<Materiel, Long> paginatorDao;
	...
	public String getPage(Integer page, Integer limit, String sort, String direction, String search) {
		...
		searchParams  = MaterielDTOConverter.convertSearchParamToEntity(search);
		this.paginatorDao.setEntityClass(Materiel.class);
		List<Materiel> list = paginatorDao.getPaginator(page, limit,sort,direction,searchParams, condition.toString());
		Long totalCount = paginatorDao.count(searchParams, condition.toString());
		List<MaterielDTO> dtos = MaterielDTOConverter.convertFromDataBeanList(list);
		return Util.toJson(new BaseTable<MaterielDTO>(dtos,totalCount));
	}
	...
	@Override
	public String advancedSearch(Integer page, Integer limit, String sort, String direction, String search, String referenceCourrier, String referenceBc, String enregistrementDateStart, String enregistrementDateEnd, String objet, String status) {
		SecUtilisateurDTO user = secUtilisateurService.getCurrentUser();
		SecProfileDTO group = secProfileService.load(user.getSecProfileId());
		...
		if(referenceBc != null && !referenceBc.equals("NAN")) {
			condition.append(" AND e.referenceBc LIKE '%").append(referenceBc.toUpperCase().replaceAll("-", "/")).append("%'");
		}
	...
	@Override
	@Transactional
	public String save(MaterielDTO dto) {
		SecUtilisateurDTO user = secUtilisateurService.getCurrentUser();
		...
		String result = ged.initiateMateriel(opentextUrl, opentextCourrierMaterielID, dto, user.getLogin(), reference, authToken);
		if(result == "FAILED") {
			return result;
		}
	...
	public MaterielDTO load(Long id) {
		Materiel entity = MaterielDao.findOne(id);
		MaterielDTO dto = MaterielDTOConverter.convertFromDataBean(entity);
	...
	@Override
	@Transactional
	public MaterielDTO loadByReference(String reference) {
		MaterielDTO dto = MaterielDTOConverter.convertFromDataBean(MaterielDao.getByReference(reference.replaceAll("-", "/")));
		return dto;
	}
	
	@Override
	@Transactional
	public MaterielDTO loadByVolumeId(Long volumeId) {
		MaterielDTO dto = MaterielDTOConverter.convertFromDataBean(MaterielDao.getByVolumeId(volumeId));
		return dto;
	}
	...
	public List<DocumentDTO> loadDocumentsByReference(String reference) {
		Materiel entity = MaterielDao.getByReference(reference.replaceAll("-", "/"));
		List<DocumentDTO> documents = new ArrayList<DocumentDTO>();
		String authToken = ged.authentication(opentextUrl, opentextLogin, opentextPassword);
		try {
			if(entity != null && entity.getwSubWork() != null) {
	...
	@Override
	@Transactional
	public String send(MaterielDTO dto) {
		SecUtilisateurDTO user = secUtilisateurService.getCurrentUser();
		String authToken = ged.authentication(opentextUrl, opentextLogin, opentextPassword);
		String result = "";
//		result = ged.getActivitiesList(opentextUrl, dto, authToken);
		if(dto.getTaskTitle().equals("Visa DAAF")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendMateriel(opentextUrl, dto, user.getLogin(), "Visa DAAF", "Motif Rejet Visa DAAF", 1, authToken);
			else
				result = ged.sendMateriel(opentextUrl, dto, user.getLogin(), "Visa DAAF", null, 1, authToken);
		}
		if(dto.getTaskTitle().equals("Traitement SM")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendMateriel(opentextUrl, dto, user.getLogin(), "Traitement SM", "Motif Rejet Traitement SM", 2, authToken);
			else
				result = ged.sendMateriel(opentextUrl, dto, user.getLogin(), "Traitement SM", null, 2, authToken);
		}
		if(dto.getTaskTitle().equals("[Rejet] Initiation"))
			result = ged.sendMateriel(opentextUrl, dto, user.getLogin(), "Rejet Initiation", null, 6, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Visa DAAF"))
			result = ged.sendMateriel(opentextUrl, dto, user.getLogin(), "Rejet Visa DAAF", null, 7, authToken);
	...
	public int getCurrentCountByEntity(String term) {
		return MaterielDao.getCurrentCountByEntity(term);
	}
	
	@Override
	@Transactional
	public int getCurrentCount() {
		return MaterielDao.getCurrentCount();
	}
	
	@Override
	@Transactional
	public int getCompletedCount() {
		return MaterielDao.getCompletedCount();
	}
	
	@Override
	@Transactional
	public int getRejectedCount() {
		return MaterielDao.getRejectedCount();
	}
	...
	public int getCurrentCountByProfile() {
		...
		return MaterielDao.getCurrentCountByProfile(profile.getPerformerName());
	}
	
	@Override
	@Transactional
	public List<Object> getCurrentCountByYear(int year) {
		return MaterielDao.getCurrentCountByYear(year);
	}
	
	@Override
	@Transactional
	public List<Object> getCompletedCountByYear(int year) {
		return MaterielDao.getCompletedCountByYear(year);
	}
	
	@Override
	@Transactional
	public int getCurrentNotOverdueClassiqueCount() {
		return MaterielDao.getCurrentNotOverdueClassiqueCount();
	}
	
	@Override
	@Transactional
	public int getCurrentOverdueClassiqueCount() {
		return MaterielDao.getCurrentOverdueClassiqueCount();
	}
	
	@Override
	@Transactional
	public int getCompletedClassiqueCount() {
		return MaterielDao.getCompletedClassiqueCount();
	}

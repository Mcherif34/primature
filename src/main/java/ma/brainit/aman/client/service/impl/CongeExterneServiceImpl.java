...
import ma.brainit.aman.client.dao.CongeExterneDao;
...
import ma.brainit.aman.client.dto.converters.CongeExterneDTOConverter;
...
@Service
@Transactional
public class CongeExterneServiceImpl implements CongeExterneService {
	...
	@Value("${opentext.courrierCongeExterneID}")
	private Long opentextCourrierCongeExterneID;
	
	private GedService ged = new GedService();
	
	@Autowired
	private CongeExterneDTOConverter CongeExterneDTOConverter;

	@Autowired
	private BasePaginatorDao<CongeExterne, Long> paginatorDao;

	@Autowired
	private CongeExterneDao CongeExterneDao;
	...
	public String getPage(Integer page, Integer limit, String sort, String direction, String search) {
		...
		searchParams  = CongeExterneDTOConverter.convertSearchParamToEntity(search);
		this.paginatorDao.setEntityClass(CongeExterne.class);
		List<CongeExterne> list = paginatorDao.getPaginator(page, limit,sort,direction,searchParams, condition.toString());
		Long totalCount = paginatorDao.count(searchParams, condition.toString());
		List<CongeExterneDTO> dtos = CongeExterneDTOConverter.convertFromDataBeanList(list);
		return Util.toJson(new BaseTable<CongeExterneDTO>(dtos,totalCount));
	}
	...
	@Override
	public String advancedSearch(Integer page, Integer limit, String sort, String direction, String search, String referenceCourrier, String referenceDemande, String enregistrementDateStart, String enregistrementDateEnd, String objet, String status) {
		SecUtilisateurDTO user = secUtilisateurService.getCurrentUser();
		SecProfileDTO group = secProfileService.load(user.getSecProfileId());
		...
		if(referenceDemande != null && !referenceDemande.equals("NAN")) {
			condition.append(" AND e.referenceDemande LIKE '%").append(referenceDemande.toUpperCase().replaceAll("-", "/")).append("%'");
		}
	...
	@Override
	@Transactional
	public String save(CongeExterneDTO dto) {
		SecUtilisateurDTO user = secUtilisateurService.getCurrentUser();
		...
		String result = ged.initiateCongeExterne(opentextUrl, opentextCourrierCongeExterneID, dto, user.getLogin(), reference, authToken);
		if(result == "FAILED") {
			return result;
		}
	...
	@Override
	@Transactional
	public CongeExterneDTO load(Long id) {
		CongeExterne entity = CongeExterneDao.findOne(id);
		CongeExterneDTO dto = CongeExterneDTOConverter.convertFromDataBean(entity);
	...
	@Override
	@Transactional
	public CongeExterneDTO loadByReference(String reference) {
		CongeExterneDTO dto = CongeExterneDTOConverter.convertFromDataBean(CongeExterneDao.getByReference(reference.replaceAll("-", "/")));
		return dto;
	}
	
	@Override
	@Transactional
	public CongeExterneDTO loadByVolumeId(Long volumeId) {
		CongeExterneDTO dto = CongeExterneDTOConverter.convertFromDataBean(CongeExterneDao.getByVolumeId(volumeId));
		return dto;
	}
	...
	public List<DocumentDTO> loadDocumentsByReference(String reference) {
		CongeExterne entity = CongeExterneDao.getByReference(reference.replaceAll("-", "/"));
		List<DocumentDTO> documents = new ArrayList<DocumentDTO>();
		String authToken = ged.authentication(opentextUrl, opentextLogin, opentextPassword);
		try {
			if(entity != null && entity.getwSubWork() != null) {
	...
	@Override
	@Transactional
	public String send(CongeExterneDTO dto) {
		SecUtilisateurDTO user = secUtilisateurService.getCurrentUser();
		String authToken = ged.authentication(opentextUrl, opentextLogin, opentextPassword);
		String result = "";
//		result = ged.getActivitiesList(opentextUrl, dto, authToken);
		if(dto.getTaskTitle().equals("Annotation SG")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendCongeExterne(opentextUrl, dto, user.getLogin(), "Annotation SG", "Motif Rejet Annotation SG", 1, authToken);
			else
				result = ged.sendCongeExterne(opentextUrl, dto, user.getLogin(), "Annotation SG", null, 1, authToken);
		}
		if(dto.getTaskTitle().equals("Traitement RH")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendCongeExterne(opentextUrl, dto, user.getLogin(), "Traitement RH", "Motif Rejet Traitement RH", 2, authToken);
			else
				result = ged.sendCongeExterne(opentextUrl, dto, user.getLogin(), "Traitement RH", null, 2, authToken);
		}
		if(dto.getTaskTitle().equals("Signature PM")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendCongeExterne(opentextUrl, dto, user.getLogin(), "Signature PM", "Motif Rejet Signature PM", 3, authToken);
			else
				result = ged.sendCongeExterne(opentextUrl, dto, user.getLogin(), "Signature PM", null, 3, authToken);
		}
		if(dto.getTaskTitle().equals("Transfert RH")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendCongeExterne(opentextUrl, dto, user.getLogin(), "Transfert RH", "Motif Rejet Transfert RH", 4, authToken);
			else
				result = ged.sendCongeExterne(opentextUrl, dto, user.getLogin(), "Transfert RH", null, 4, authToken);
		}
		if(dto.getTaskTitle().equals("[Rejet] Initiation"))
			result = ged.sendCongeExterne(opentextUrl, dto, user.getLogin(), "Rejet Initiation", null, 10, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Annotation SG"))
			result = ged.sendCongeExterne(opentextUrl, dto, user.getLogin(), "Rejet Annotation SG", null, 11, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Signature PM"))
			result = ged.sendCongeExterne(opentextUrl, dto, user.getLogin(), "Rejet Signature PM", null, 12, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Traitement RH"))
			result = ged.sendCongeExterne(opentextUrl, dto, user.getLogin(), "Rejet Traitement RH", null, 13, authToken);
	...
	@Override
	@Transactional
	public List<CongeExterneDTO> getAll() {
		List<CongeExterne> list = CongeExterneDao.findAll();
		List<CongeExterneDTO> dtos = CongeExterneDTOConverter.convertFromDataBeanList(list);
	...
	public int getCurrentCountByEntity(String term) {
		return CongeExterneDao.getCurrentCountByEntity(term);
	}
	
	@Override
	@Transactional
	public int getCurrentCount() {
		return CongeExterneDao.getCurrentCount();
	}
	
	@Override
	@Transactional
	public int getCompletedCount() {
		return CongeExterneDao.getCompletedCount();
	}
	
	@Override
	@Transactional
	public int getRejectedCount() {
		return CongeExterneDao.getRejectedCount();
	}
	...
	public int getCurrentCountByProfile() {
		...
		return CongeExterneDao.getCurrentCountByProfile(profile.getPerformerName());
	}
	
	@Override
	@Transactional
	public List<Object> getCurrentCountByYear(int year) {
		return CongeExterneDao.getCurrentCountByYear(year);
	}
	
	@Override
	@Transactional
	public List<Object> getCompletedCountByYear(int year) {
		return CongeExterneDao.getCompletedCountByYear(year);
	}
	
	@Override
	@Transactional
	public int getCurrentNotOverdueClassiqueCount() {
		return CongeExterneDao.getCurrentNotOverdueClassiqueCount();
	}
	
	@Override
	@Transactional
	public int getCurrentOverdueClassiqueCount() {
		return CongeExterneDao.getCurrentOverdueClassiqueCount();
	}
	
	@Override
	@Transactional
	public int getCompletedClassiqueCount() {
		return CongeExterneDao.getCompletedClassiqueCount();
	}

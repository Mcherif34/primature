...
import ma.brainit.aman.client.dao.AudienceSgDao;
...
import ma.brainit.aman.client.dto.converters.AudienceSgDTOConverter;
...
import ma.brainit.aman.client.service.AudienceSgService;
...
@Service
@Transactional
public class AudienceSgServiceImpl implements AudienceSgService {
	...
	@Value("${opentext.courrierAudienceSgID}")
	private Long opentextCourrierAudienceSgID;
	
	private GedService ged = new GedService();
	
	@Autowired
	private AudienceSgDTOConverter AudienceDTOConverter;

	@Autowired
	private BasePaginatorDao<AudienceSg, Long> paginatorDao;

	@Autowired
	private AudienceSgDao AudienceDao;
	...
	public String getPage(Integer page, Integer limit, String sort, String direction, String search) {
		...
		searchParams  = AudienceDTOConverter.convertSearchParamToEntity(search);
		this.paginatorDao.setEntityClass(AudienceSg.class);
		List<AudienceSg> list = paginatorDao.getPaginator(page, limit,sort,direction,searchParams, condition.toString());
		Long totalCount = paginatorDao.count(searchParams, condition.toString());
		List<AudienceSgDTO> dtos = AudienceDTOConverter.convertFromDataBeanList(list);
//		for(AudienceDTO AudienceDTO : dtos) {
//			WSubWorkTask wSubWorkTask = wSubWorkTaskDao.getByWWork(AudienceDTO.getwSubWorkId());
//			if(wSubWorkTask != null) {
//				AudienceDTO.setTaskTitle(wSubWorkTask.getSubWorkTaskTitle());
//				AudienceDTO.setPerformer(wSubWorkTask.getPerformer().getName());
//			}
//		}
		return Util.toJson(new BaseTable<AudienceSgDTO>(dtos,totalCount));
	}
...
//		}
	...
	@Override
	@Transactional
	public String save(AudienceSgDTO dto) {
		SecUtilisateurDTO user = secUtilisateurService.getCurrentUser();
		...
		String result = ged.initiateAudienceSg(opentextUrl, opentextCourrierAudienceSgID, dto, user.getLogin(), reference, authToken);
		if(result == "FAILED") {
			return result;
		}
	...
	@Override
	@Transactional
	public AudienceSgDTO load(Long id) {
		AudienceSg entity = AudienceDao.findOne(id);
		AudienceSgDTO dto = AudienceDTOConverter.convertFromDataBean(entity);
	...
	@Override
	@Transactional
	public AudienceSgDTO loadByReference(String reference) {
		AudienceSgDTO dto = AudienceDTOConverter.convertFromDataBean(AudienceDao.getByReference(reference.replaceAll("-", "/")));
		return dto;
	}
	
	@Override
	@Transactional
	public AudienceSgDTO loadByVolumeId(Long volumeId) {
		AudienceSgDTO dto = AudienceDTOConverter.convertFromDataBean(AudienceDao.getByVolumeId(volumeId));
		return dto;
	}
	...
	public List<DocumentDTO> loadDocumentsByReference(String reference) {
		AudienceSg entity = AudienceDao.getByReference(reference.replaceAll("-", "/"));
		List<DocumentDTO> documents = new ArrayList<DocumentDTO>();
		String authToken = ged.authentication(opentextUrl, opentextLogin, opentextPassword);
		try {
			if(entity != null && entity.getwSubWork() != null) {
	...
	@Override
	@Transactional
	public String send(AudienceSgDTO dto) {
		SecUtilisateurDTO user = secUtilisateurService.getCurrentUser();
		String authToken = ged.authentication(opentextUrl, opentextLogin, opentextPassword);
		String result = "";
//		result = ged.getActivitiesList(opentextUrl, dto, authToken);
		if(dto.getTaskTitle().equals("Réception du courrier")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Initiation", "Motif Rejet Courrier", 1, authToken);
			else
				result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Initiation", null, 1, authToken);
		}
		if(dto.getTaskTitle().equals("Annotation SG")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Annotation SG", "Motif Rejet Annotation SG", 6, authToken);
			else
				result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Annotation SG", null, 6, authToken);
		}
		if(dto.getTaskTitle().equals("Vérification Coordonnateur")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Vérification Coordonnateur", "Motif Rejet Vérification Coordonnateur", 7, authToken);
			else
				result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Vérification Coordonnateur", null, 7, authToken);
		}
		if(dto.getTaskTitle().equals("Validation SG")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Validation SG", "Motif Rejet Validation SG", 9, authToken);
			else
				result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Validation SG", null, 9, authToken);
		}
		if(dto.getTaskTitle().equals("Préparation Conseiller")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Préparation Conseiller", "Motif Rejet Préparation Conseiller", 11, authToken);
			else
				result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Préparation Conseiller", null, 11, authToken);
		}
		if(dto.getTaskTitle().equals("Transfert SG")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Transfert SG", "Motif Rejet Transfert SG", 13, authToken);
			else
				result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Transfert SG", null, 13, authToken);
		}
		if(dto.getTaskTitle().equals("Consultation PM")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Consultation PM", "Motif Rejet Consultation PM", 15, authToken);
			else
				result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Consultation PM", null, 15, authToken);
		}
		if(dto.getTaskTitle().equals("[Rejet] Initiation"))
			result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Rejet Initiation", null, 4, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Réception du courrier"))
			result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Rejet Réception du courrier", null, 5, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Annotation SG"))
			result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Rejet Annotation SG", null, 18, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Vérification Coordonnateur"))
			result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Rejet Vérification Coordonnateur", null, 19, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Validation SG"))
			result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Rejet Validation SG", null, 20, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Préparation Conseiller"))
			result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Rejet Préparation Conseiller", null, 21, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Transfert SG"))
			result = ged.sendAudienceSg(opentextUrl, dto, user.getLogin(), "Rejet Transfert SG", null, 22, authToken);
	...
	@Override
	@Transactional
	public List<AudienceSgDTO> getAll() {
		List<AudienceSg> list = AudienceDao.findAll();
		List<AudienceSgDTO> dtos = AudienceDTOConverter.convertFromDataBeanList(list);
	...
	public int getCurrentCountByEntity(String term) {
		return AudienceDao.getCurrentCountByEntity(term);
	}
	
	@Override
	@Transactional
	public int getCurrentCount() {
		return AudienceDao.getCurrentCount();
	}
	
	@Override
	@Transactional
	public int getCompletedCount() {
		return AudienceDao.getCompletedCount();
	}
	
	@Override
	@Transactional
	public int getRejectedCount() {
		return AudienceDao.getRejectedCount();
	}
	...
	public int getCurrentCountByProfile() {
		...
		return AudienceDao.getCurrentCountByProfile(profile.getPerformerName());
	}
	
	@Override
	@Transactional
	public List<Object> getCurrentCountByYear(int year) {
		return AudienceDao.getCurrentCountByYear(year);
	}
	
	@Override
	@Transactional
	public List<Object> getCompletedCountByYear(int year) {
		return AudienceDao.getCompletedCountByYear(year);
	}
	
	@Override
	@Transactional
	public int getCurrentNotOverdueClassiqueCount() {
		return AudienceDao.getCurrentNotOverdueClassiqueCount();
	}
	
	@Override
	@Transactional
	public int getCurrentOverdueClassiqueCount() {
		return AudienceDao.getCurrentOverdueClassiqueCount();
	}
	
	@Override
	@Transactional
	public int getCompletedClassiqueCount() {
		return AudienceDao.getCompletedClassiqueCount();
	}

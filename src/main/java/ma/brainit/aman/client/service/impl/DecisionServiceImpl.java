...
import ma.brainit.aman.client.dao.DecisionDao;
...
import ma.brainit.aman.client.dto.converters.DecisionDTOConverter;
...
public class DecisionServiceImpl implements DecisionService {
	...
	@Value("${opentext.courrierDecisionID}")
	private Long opentextCourrierDecisionID;
	...
	@Autowired
	private BasePaginatorDao<Decision, Long> paginatorDao;
	...
	public String getPage(Integer page, Integer limit, String sort, String direction, String search) {
		...
		searchParams  = DecisionDTOConverter.convertSearchParamToEntity(search);
		this.paginatorDao.setEntityClass(Decision.class);
		List<Decision> list = paginatorDao.getPaginator(page, limit,sort,direction,searchParams, condition.toString());
		Long totalCount = paginatorDao.count(searchParams, condition.toString());
		List<DecisionDTO> dtos = DecisionDTOConverter.convertFromDataBeanList(list);
		return Util.toJson(new BaseTable<DecisionDTO>(dtos,totalCount));
	}
	...
	@Override
	public String advancedSearch(Integer page, Integer limit, String sort, String direction, String search, String referenceCourrier, String referenceDecision, String enregistrementDateStart, String enregistrementDateEnd, String objet, String status) {
		SecUtilisateurDTO user = secUtilisateurService.getCurrentUser();
		SecProfileDTO group = secProfileService.load(user.getSecProfileId());
		...
		if(referenceDecision != null && !referenceDecision.equals("NAN")) {
			condition.append(" AND e.referenceDecision LIKE '%").append(referenceDecision.toUpperCase().replaceAll("-", "/")).append("%'");
		}
	...
	@Override
	@Transactional
	public String save(DecisionDTO dto) {
		SecUtilisateurDTO user = secUtilisateurService.getCurrentUser();
		...
		String result = ged.initiateDecision(opentextUrl, opentextCourrierDecisionID, dto, user.getLogin(), reference, authToken);
		if(result == "FAILED") {
			return result;
		}
	...
	@Override
	@Transactional
	public DecisionDTO load(Long id) {
		Decision entity = DecisionDao.findOne(id);
		DecisionDTO dto = DecisionDTOConverter.convertFromDataBean(entity);
	...
	@Override
	@Transactional
	public DecisionDTO loadByReference(String reference) {
		DecisionDTO dto = DecisionDTOConverter.convertFromDataBean(DecisionDao.getByReference(reference.replaceAll("-", "/")));
		return dto;
	}
	
	@Override
	@Transactional
	public DecisionDTO loadByVolumeId(Long volumeId) {
		DecisionDTO dto = DecisionDTOConverter.convertFromDataBean(DecisionDao.getByVolumeId(volumeId));
		return dto;
	}
	...
	public List<DocumentDTO> loadDocumentsByReference(String reference) {
		Decision entity = DecisionDao.getByReference(reference.replaceAll("-", "/"));
		List<DocumentDTO> documents = new ArrayList<DocumentDTO>();
		String authToken = ged.authentication(opentextUrl, opentextLogin, opentextPassword);
		try {
			if(entity != null && entity.getwSubWork() != null) {
	...
	@Override
	@Transactional
	public String send(DecisionDTO dto) {
		SecUtilisateurDTO user = secUtilisateurService.getCurrentUser();
		String authToken = ged.authentication(opentextUrl, opentextLogin, opentextPassword);
		String result = "";
//		result = ged.getActivitiesList(opentextUrl, dto, authToken);
		if(dto.getTaskTitle().equals("Transfert SG")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendDecision(opentextUrl, dto, user.getLogin(), "Transfert SG", "Motif Rejet Transfert SG", 1, authToken);
			else
				result = ged.sendDecision(opentextUrl, dto, user.getLogin(), "Transfert SG", null, 1, authToken);
		}
		if(dto.getTaskTitle().equals("Vérification Coordonnateur")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendDecision(opentextUrl, dto, user.getLogin(), "Vérification Coordonnateur", "Motif Rejet Vérification Coordonnateur", 2, authToken);
			else
				result = ged.sendDecision(opentextUrl, dto, user.getLogin(), "Vérification Coordonnateur", null, 2, authToken);
		}
		if(dto.getTaskTitle().equals("Visa SG")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendDecision(opentextUrl, dto, user.getLogin(), "Visa SG", "Motif Rejet Visa SG", 3, authToken);
			else
				result = ged.sendDecision(opentextUrl, dto, user.getLogin(), "Visa SG", null, 3, authToken);
		}
		if(dto.getTaskTitle().equals("Signature PM")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendDecision(opentextUrl, dto, user.getLogin(), "Signature PM", "Motif Rejet Signature PM", 4, authToken);
			else
				result = ged.sendDecision(opentextUrl, dto, user.getLogin(), "Signature PM", null, 4, authToken);
		}
		if(dto.getTaskTitle().equals("Prise en charge RH")) {
			if(dto.getMotif() != null && dto.getMotif() != "")
				result = ged.sendDecision(opentextUrl, dto, user.getLogin(), "Prise en charge RH", "Motif Rejet Prise en charge RH", 5, authToken);
			else
				result = ged.sendDecision(opentextUrl, dto, user.getLogin(), "Prise en charge RH", null, 5, authToken);
		}
		if(dto.getTaskTitle().equals("[Rejet] Initiation"))
			result = ged.sendDecision(opentextUrl, dto, user.getLogin(), "Rejet Initiation", null, 8, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Transfert SG"))
			result = ged.sendDecision(opentextUrl, dto, user.getLogin(), "Rejet Transfert SG", null, 13, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Vérification Coordonnateur"))
			result = ged.sendDecision(opentextUrl, dto, user.getLogin(), "Rejet Vérification Coordonnateur", null, 14, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Visa SG"))
			result = ged.sendDecision(opentextUrl, dto, user.getLogin(), "Rejet Visa SG", null, 15, authToken);
		if(dto.getTaskTitle().equals("[Rejet] Signature PM"))
			result = ged.sendDecision(opentextUrl, dto, user.getLogin(), "Rejet Signature PM", null, 16, authToken);
	...
	public List<DecisionDTO> getAll() {
		List<Decision> list = DecisionDao.findAll();
		List<DecisionDTO> dtos = DecisionDTOConverter.convertFromDataBeanList(list);
	...
	public int getCurrentCountByEntity(String term) {
		return DecisionDao.getCurrentCountByEntity(term);
	}
	
	@Override
	@Transactional
	public int getCurrentCount() {
		return DecisionDao.getCurrentCount();
	}
	
	@Override
	@Transactional
	public int getCompletedCount() {
		return DecisionDao.getCompletedCount();
	}
	
	@Override
	@Transactional
	public int getRejectedCount() {
		return DecisionDao.getRejectedCount();
	}
	...
	public int getCurrentCountByProfile() {
		...
		return DecisionDao.getCurrentCountByProfile(profile.getPerformerName());
	}
	
	@Override
	@Transactional
	public List<Object> getCurrentCountByYear(int year) {
		return DecisionDao.getCurrentCountByYear(year);
	}
	
	@Override
	@Transactional
	public List<Object> getCompletedCountByYear(int year) {
		return DecisionDao.getCompletedCountByYear(year);
	}
	
	@Override
	@Transactional
	public int getCurrentNotOverdueClassiqueCount() {
		return DecisionDao.getCurrentNotOverdueClassiqueCount();
	}
	
	@Override
	@Transactional
	public int getCurrentOverdueClassiqueCount() {
		return DecisionDao.getCurrentOverdueClassiqueCount();
	}
	
	@Override
	@Transactional
	public int getCompletedClassiqueCount() {
		return DecisionDao.getCompletedClassiqueCount();
	}

app.controller('courrierController', function($scope, $http, $ngBootbox, $location, CRUDService, NotificationService, constants) {
	...
	$scope.initIndicators = function() {
		$http.get(context+$scope.baseUrl+"getCurrentCountByProfile").success(function(data, status) {   
			$scope.currentNominationCountByProfile = data;
			$scope.taskByProfile = $scope.currentNominationCountByProfile;
		});
	}
	...
	$scope.advancedSearchCurrentNomination = function() {
		var searchReferenceCourrier = 'NAN'; var searchReferenceNomination = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchObjet = 'NAN';
		...
		if(!angular.isUndefined($scope.dto.search.referenceNominationCurrent) && $scope.dto.search.referenceNominationCurrent != '') searchReferenceNomination = $scope.dto.search.referenceNominationCurrent.replaceAll("/", "-");
		...
		$scope.courrierCurrentTable.ajax.url($scope.baseUrl+'search/'+searchReferenceCourrier+'/'+searchReferenceNomination+'/'+searchEnregistrementDateStart+'/'+searchEnregistrementDateEnd+'/'+searchObjet+'/current').load();
	};
	
	$scope.advancedSearchDoneNomination = function() {
		var searchReferenceCourrier = 'NAN'; var searchReferenceNomination = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchObjet = 'NAN';
		...
		if(!angular.isUndefined($scope.dto.search.referenceNominationDone) && $scope.dto.search.referenceNominationDone != '') searchReferenceNomination = $scope.dto.search.referenceNominationDone.replaceAll("/", "-");
		...
		$scope.courrierDoneTable.ajax.url($scope.baseUrl+'search/'+searchReferenceCourrier+'/'+searchReferenceNomination+'/'+searchEnregistrementDateStart+'/'+searchEnregistrementDateEnd+'/'+searchObjet+'/done').load();
	};
	
	$scope.advancedSearchCompletedNomination = function() {
		var searchReferenceCourrier = 'NAN'; var searchReferenceNomination = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchObjet = 'NAN';
		...
		if(!angular.isUndefined($scope.dto.search.referenceNominationCompleted) && $scope.dto.search.referenceNominationCompleted != '') searchReferenceNomination = $scope.dto.search.referenceNominationCompleted.replaceAll("/", "-");
		...
		$scope.courrierCloseTable.ajax.url($scope.baseUrl+'search/'+searchReferenceCourrier+'/'+searchReferenceNomination+'/'+searchEnregistrementDateStart+'/'+searchEnregistrementDateEnd+'/'+searchObjet+'/completed').load();
	};
	...
	$scope.saveConfirmCourrier = function() {
		...
		formData.append("referenceNomination", $scope.dto.referenceNomination);
		...
		$http({
			url: context+"/nomination/rest/save",
			method: 'POST',
			data: formData,
			...
			if(data != "FAILED") {
				$scope.refreshList();
				Toastify({
					text: "La nomination a été bien enregistrée sous la référence Nº "+data,
				...
				Toastify({
					text: "Impossible d'enregistrer la nomination. Veuillez contacter l'administrateur du système.",
	...
	$scope.confirmRemoveCourrier = function() {
		...
		$http.post(context+$scope.baseUrl+"delete", angular.toJson($scope.wSubWorkId)).success(function(data, status) {
			...
			$('#removeCourrierModal').modal("hide");
			Toastify({
				text: "Impossible de supprimer la nomination. Veuillez contacter l'administrateur du système.",
				gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
			}).showToast();
	});
	};
	...
	$scope.confirmSendCourrier = function() {
		...
		$http({
			url: context+"/nomination/rest/send",
			method: 'POST',
			data: formData,
			headers: {'Content-Type': undefined},
			transformRequest: angular.identity
			...
			if(data == "SUCCESS") {
				$scope.refreshList();
				Toastify({
					text: "La nomination a été transférée avec succès.",
					gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
				}).showToast();
			} else {
				Toastify({
					text: "Impossible de transférer la nomination. Veuillez contacter l'administrateur du système.",
					gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #e84e4e, #a33737)"}
				}).showToast();
			}
		}).error(function(data, status, headers, config) {
		...
		if($scope.cancelCourrierForm.$valid) {
			...
			$http({
				...
				if(data == "SUCCESS") {
					$scope.refreshList();
					Toastify({
						text: "La nomination a été rejetée avec succès.",
						gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
					}).showToast();
				} else {
					Toastify({
						text: "Impossible de rejeter la nomination. Veuillez contacter l'administrateur du système.",
						gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #e84e4e, #a33737)"}
					}).showToast();
				}
			}).error(function(data, status, headers, config) {
	...
	$scope.exportCourriers = function(name) {
		...
		$scope.courrierTab = [];
		var title = ["REFERENCE COURRIER", "DATE ENREGISTREMENT", "REFERENCE NOMINATION", "OBJET", "TRANSFERE A", "STATUT"];
		...
		for(var i = 0, l = inputs.length; i < l; ++i) {
			if(inputs[i].checked) {
				$http.get(context+$scope.baseUrl+"loadByReference/"+inputs[i].value.replaceAll("/", "-")).success(function(data, status) {
					...
					courrier[2] = data.referenceNomination;
	...
	$scope.downloadFile = function() {
		exportExcelFile($scope.courrierTab, 'Nominations.xlsx', 'Nominations');
	}

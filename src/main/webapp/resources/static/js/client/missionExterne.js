app.controller('courrierController', function($scope, $http, $ngBootbox, $location, CRUDService, NotificationService, constants) {
	...
	$scope.baseUrl = "/missionexterne/rest/";
	...
	$scope.initIndicators = function() {
		$http.get(context+$scope.baseUrl+"getCurrentCountByProfile").success(function(data, status) {   
			$scope.currentMissionExterneCountByProfile = data;
			$scope.taskByProfile = $scope.currentMissionExterneCountByProfile;
		});
	}
	...
	$scope.advancedSearchCurrentMissionExterne = function() {
	...
	$scope.advancedSearchDoneMissionExterne = function() {
		var searchReferenceCourrier = 'NAN'; var searchReceptionDateStart = 'NAN'; var searchReceptionDateEnd = 'NAN'; var searchReferenceExpediteur = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchExpediteur = 'NAN'; var searchObjet = 'NAN';
	...
	$scope.advancedSearchCompletedMissionExterne = function() {
		var searchReferenceCourrier = 'NAN'; var searchReceptionDateStart = 'NAN'; var searchReceptionDateEnd = 'NAN'; var searchReferenceExpediteur = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchExpediteur = 'NAN'; var searchObjet = 'NAN';
	...
	$scope.saveConfirmCourrier = function() {
		...
		$http({
			url: context+"/missionexterne/rest/save",
			method: 'POST',
			data: formData,
			...
			if(data != "FAILED") {
				$scope.refreshList();
				Toastify({
					text: "L'ordre de mission a été bien enregistré sous la référence Nº "+data,
				...
				Toastify({
					text: "Impossible d'enregistrer l'ordre de mission. Veuillez contacter l'administrateur du système.",
	...
	$scope.confirmRemoveCourrier = function() {
		...
		$http.post(context+$scope.baseUrl+"delete", angular.toJson($scope.wSubWorkId)).success(function(data, status) {
			...
			$scope.refreshList();
			Toastify({
				text: "L'ordre de mission a été supprimé avec succès.",
				gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
			}).showToast();
		}).error(function(data, status, headers, config) {
			$scope.removeProgress = false;
			$('#removeCourrierModal').modal("hide");
			Toastify({
				text: "Impossible de supprimer l'ordre de mission. Veuillez contacter l'administrateur du système.",
				gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
			}).showToast();
	});
	};
	...
	$scope.confirmSendCourrier = function() {
		...
		$http({
			url: context+"/missionexterne/rest/send",
			method: 'POST',
			data: formData,
			headers: {'Content-Type': undefined},
			transformRequest: angular.identity
			...
			if(data == "SUCCESS") {
				$scope.refreshList();
				Toastify({
					text: "L'ordre de mission a été transféré avec succès.",
					gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
				}).showToast();
			} else {
				Toastify({
					text: "Impossible de transférer l'ordre de mission. Veuillez contacter l'administrateur du système.",
					gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #e84e4e, #a33737)"}
				}).showToast();
			}
		}).error(function(data, status, headers, config) {
		...
		if($scope.cancelCourrierForm.$valid) {
			...
			$http({
				...
				if(data == "SUCCESS") {
					$scope.refreshList();
					Toastify({
						text: "L'ordre de mission a été rejeté avec succès.",
						gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
					}).showToast();
				} else {
					Toastify({
						text: "Impossible de rejeter l'ordre de mission. Veuillez contacter l'administrateur du système.",
						gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #e84e4e, #a33737)"}
					}).showToast();
				}
			}).error(function(data, status, headers, config) {
	...
	$scope.downloadFile = function() {
		exportExcelFile($scope.courrierTab, 'Ordres de mission.xlsx', 'Ordres de mission');
	}

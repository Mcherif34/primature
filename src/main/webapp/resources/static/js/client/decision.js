app.controller('courrierController', function($scope, $http, $ngBootbox, $location, CRUDService, NotificationService, constants) {
	...
	$scope.initIndicators = function() {
		$http.get(context+$scope.baseUrl+"getCurrentCountByProfile").success(function(data, status) {   
			$scope.currentDecisionCountByProfile = data;
			$scope.taskByProfile = $scope.currentDecisionCountByProfile;
		});
	}
	...
	$scope.advancedSearchCurrentDecision = function() {
		var searchReferenceCourrier = 'NAN'; var searchReferenceDecision = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchObjet = 'NAN';
		...
		if(!angular.isUndefined($scope.dto.search.referenceDecisionCurrent) && $scope.dto.search.referenceDecisionCurrent != '') searchReferenceDecision = $scope.dto.search.referenceDecisionCurrent.replaceAll("/", "-");
		...
		$scope.courrierCurrentTable.ajax.url($scope.baseUrl+'search/'+searchReferenceCourrier+'/'+searchReferenceDecision+'/'+searchEnregistrementDateStart+'/'+searchEnregistrementDateEnd+'/'+searchObjet+'/current').load();
	};
	
	$scope.advancedSearchDoneDecision = function() {
		var searchReferenceCourrier = 'NAN'; var searchReferenceDecision = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchObjet = 'NAN';
		...
		if(!angular.isUndefined($scope.dto.search.referenceDecisionDone) && $scope.dto.search.referenceDecisionDone != '') searchReferenceDecision = $scope.dto.search.referenceDecisionDone.replaceAll("/", "-");
		...
		$scope.courrierDoneTable.ajax.url($scope.baseUrl+'search/'+searchReferenceCourrier+'/'+searchReferenceDecision+'/'+searchEnregistrementDateStart+'/'+searchEnregistrementDateEnd+'/'+searchObjet+'/done').load();
	};
	
	$scope.advancedSearchCompletedDecision = function() {
		var searchReferenceCourrier = 'NAN'; var searchReferenceDecision = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchObjet = 'NAN';
		...
		if(!angular.isUndefined($scope.dto.search.referenceDecisionCompleted) && $scope.dto.search.referenceDecisionCompleted != '') searchReferenceDecision = $scope.dto.search.referenceDecisionCompleted.replaceAll("/", "-");
		...
		$scope.courrierCloseTable.ajax.url($scope.baseUrl+'search/'+searchReferenceCourrier+'/'+searchReferenceDecision+'/'+searchEnregistrementDateStart+'/'+searchEnregistrementDateEnd+'/'+searchObjet+'/completed').load();
	};
	...
	$scope.saveConfirmCourrier = function() {
		...
		formData.append("referenceDecision", $scope.dto.referenceDecision);
		...
		$http({
			url: context+"/decision/rest/save",
			method: 'POST',
			data: formData,
	...
	$scope.confirmSendCourrier = function() {
		...
		$http({
			url: context+"/decision/rest/send",
			method: 'POST',
			data: formData,
			headers: {'Content-Type': undefined},
			transformRequest: angular.identity
	...
	$scope.exportCourriers = function(name) {
		...
		$scope.courrierTab = [];
		var title = ["REFERENCE COURRIER", "DATE ENREGISTREMENT", "REFERENCE DECISION", "OBJET", "TRANSFERE A", "STATUT"];
		...
		for(var i = 0, l = inputs.length; i < l; ++i) {
			if(inputs[i].checked) {
				$http.get(context+$scope.baseUrl+"loadByReference/"+inputs[i].value.replaceAll("/", "-")).success(function(data, status) {
					...
					courrier[2] = data.referenceDecision;
	...
	$scope.downloadFile = function() {
		exportExcelFile($scope.courrierTab, 'Decisions.xlsx', 'Decisions');
	}

app.controller('courrierController', function($scope, $http, $ngBootbox, $location, CRUDService, NotificationService, constants) {
	...
	$scope.initIndicators = function() {
		$http.get(context+$scope.baseUrl+"getCurrentCountByProfile").success(function(data, status) {   
			$scope.currentMaterielCountByProfile = data;
			$scope.taskByProfile = $scope.currentMaterielCountByProfile;
		});
	}
	...
	$scope.advancedSearchCurrentMateriel = function() {
		var searchReferenceCourrier = 'NAN'; var searchReferenceBc = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchObjet = 'NAN';
		...
		if(!angular.isUndefined($scope.dto.search.referenceBcCurrent) && $scope.dto.search.referenceBcCurrent != '') searchReferenceBc = $scope.dto.search.referenceBcCurrent.replaceAll("/", "-");
		...
		$scope.courrierCurrentTable.ajax.url($scope.baseUrl+'search/'+searchReferenceCourrier+'/'+searchReferenceBc+'/'+searchEnregistrementDateStart+'/'+searchEnregistrementDateEnd+'/'+searchObjet+'/current').load();
	};
	
	$scope.advancedSearchDoneMateriel = function() {
		var searchReferenceCourrier = 'NAN'; var searchReferenceBc = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchObjet = 'NAN';
		...
		if(!angular.isUndefined($scope.dto.search.referenceBcDone) && $scope.dto.search.referenceBcDone != '') searchReferenceBc = $scope.dto.search.referenceBcDone.replaceAll("/", "-");
		...
		$scope.courrierDoneTable.ajax.url($scope.baseUrl+'search/'+searchReferenceCourrier+'/'+searchReferenceBc+'/'+searchEnregistrementDateStart+'/'+searchEnregistrementDateEnd+'/'+searchObjet+'/done').load();
	};
	
	$scope.advancedSearchCompletedMateriel = function() {
		var searchReferenceCourrier = 'NAN'; var searchReferenceBc = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchObjet = 'NAN';
		...
		if(!angular.isUndefined($scope.dto.search.referenceBcCompleted) && $scope.dto.search.referenceBcCompleted != '') searchReferenceBc = $scope.dto.search.referenceBcCompleted.replaceAll("/", "-");
		...
		$scope.courrierCloseTable.ajax.url($scope.baseUrl+'search/'+searchReferenceCourrier+'/'+searchReferenceBc+'/'+searchEnregistrementDateStart+'/'+searchEnregistrementDateEnd+'/'+searchObjet+'/completed').load();
	};
	...
	$scope.saveConfirmCourrier = function() {
		...
		formData.append("referenceBc", $scope.dto.referenceBc);
		...
		$http({
			url: context+"/materiel/rest/save",
			method: 'POST',
			data: formData,
			...
			if(data != "FAILED") {
				$scope.refreshList();
				Toastify({
					text: "Le bon de commande a été bien enregistré sous la référence Nº "+data,
				...
				Toastify({
					text: "Impossible d'enregistrer le bon de commande. Veuillez contacter l'administrateur du système.",
	...
	$scope.confirmRemoveCourrier = function() {
		...
		$http.post(context+$scope.baseUrl+"delete", angular.toJson($scope.wSubWorkId)).success(function(data, status) {
			...
			$scope.refreshList();
			Toastify({
				text: "Le bon de commande a été supprimée avec succès.",
				gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
			}).showToast();
		}).error(function(data, status, headers, config) {
			$scope.removeProgress = false;
			$('#removeCourrierModal').modal("hide");
			Toastify({
				text: "Impossible de supprimer le bon de commande. Veuillez contacter l'administrateur du système.",
				gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
			}).showToast();
	});
	};
	...
	$scope.confirmSendCourrier = function() {
		...
		$http({
			url: context+"/materiel/rest/send",
			method: 'POST',
			data: formData,
			headers: {'Content-Type': undefined},
			transformRequest: angular.identity
			...
			if(data == "SUCCESS") {
				$scope.refreshList();
				Toastify({
					text: "Le bon de commande a été transféré avec succès.",
					gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
				}).showToast();
			} else {
				Toastify({
					text: "Impossible de transférer le bon de commande. Veuillez contacter l'administrateur du système.",
					gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #e84e4e, #a33737)"}
				}).showToast();
			}
		}).error(function(data, status, headers, config) {
		...
		if($scope.cancelCourrierForm.$valid) {
			...
			$http({
				...
				if(data == "SUCCESS") {
					$scope.refreshList();
					Toastify({
						text: "Le bon de commande a été rejeté avec succès.",
						gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
					}).showToast();
				} else {
					Toastify({
						text: "Impossible de rejeter le bon de commande. Veuillez contacter l'administrateur du système.",
						gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #e84e4e, #a33737)"}
					}).showToast();
				}
			}).error(function(data, status, headers, config) {
	...
	$scope.exportCourriers = function(name) {
		...
		$scope.courrierTab = [];
		var title = ["REFERENCE COURRIER", "DATE ENREGISTREMENT", "REFERENCE BON DE COMMANDE", "OBJET", "TRANSFERE A", "STATUT"];
	...
	$scope.downloadFile = function() {
		exportExcelFile($scope.courrierTab, 'Materiels.xlsx', 'Materiels');
	}

app.controller('courrierController', function($scope, $http, $ngBootbox, $location, CRUDService, NotificationService, constants) {
	...
	$scope.baseUrl = "/missioninterne/rest/";
	...
	$scope.init = function() {
		...
		$scope.transfereAu = [
			{
			  id: 'Direction de Cabinet du Premier Ministre',
			  name: 'Direction de Cabinet du Premier Ministre'
			}, {
			  id: 'Secrétariat Général',
	...
	$scope.initIndicators = function() {
		$http.get(context+$scope.baseUrl+"getCurrentCountByProfile").success(function(data, status) {   
			$scope.currentMissionInterneCountByProfile = data;
			$scope.taskByProfile = $scope.currentMissionInterneCountByProfile;
		});
	}
	...
	$scope.advancedSearchCurrentMissionInterne = function() {
		var searchReferenceCourrier = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchObjet = 'NAN';
		...
		$scope.courrierCurrentTable.ajax.url($scope.baseUrl+'search/'+searchReferenceCourrier+'/'+searchEnregistrementDateStart+'/'+searchEnregistrementDateEnd+'/'+searchObjet+'/current').load();
	};
	
	$scope.advancedSearchDoneMissionInterne = function() {
		var searchReferenceCourrier = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchObjet = 'NAN';
		...
		$scope.courrierDoneTable.ajax.url($scope.baseUrl+'search/'+searchReferenceCourrier+'/'+searchEnregistrementDateStart+'/'+searchEnregistrementDateEnd+'/'+searchObjet+'/done').load();
	};
	
	$scope.advancedSearchCompletedMissionInterne = function() {
		var searchReferenceCourrier = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchObjet = 'NAN';
		...
		$scope.courrierCloseTable.ajax.url($scope.baseUrl+'search/'+searchReferenceCourrier+'/'+searchEnregistrementDateStart+'/'+searchEnregistrementDateEnd+'/'+searchObjet+'/completed').load();
	};
	...
	$scope.saveConfirmCourrier = function() {
		...
		$http({
			url: context+"/missioninterne/rest/save",
			method: 'POST',
			data: formData,
	...
	$scope.confirmSendCourrier = function() {
		...
		$http({
			url: context+"/missioninterne/rest/send",
			method: 'POST',
			data: formData,
			headers: {'Content-Type': undefined},
			transformRequest: angular.identity
	...
	$scope.exportCourriers = function(name) {
		...
		$scope.courrierTab = [];
		var title = ["REFERENCE COURRIER", "DATE ENREGISTREMENT", "OBJET", "TRANSFERE A", "STATUT"];
		...
		for(var i = 0, l = inputs.length; i < l; ++i) {
			if(inputs[i].checked) {
				$http.get(context+$scope.baseUrl+"loadByReference/"+inputs[i].value.replaceAll("/", "-")).success(function(data, status) {
					...
					courrier[2] = data.objet;
					courrier[3] = data.taskTitle;
					var status = "";

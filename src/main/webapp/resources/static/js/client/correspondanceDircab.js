app.controller('courrierController', function($scope, $http, $ngBootbox, $location, CRUDService, NotificationService, constants) {
	...
	$scope.baseUrl = "/correspondancedircab/rest/";
	...
	$scope.advancedSearchCurrentCorrespondance = function() {
	...
	$scope.advancedSearchDoneCorrespondance = function() {
		var searchReferenceCourrier = 'NAN'; var searchReceptionDateStart = 'NAN'; var searchReceptionDateEnd = 'NAN'; var searchReferenceExpediteur = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchExpediteur = 'NAN'; var searchObjet = 'NAN';
	...
	$scope.advancedSearchCompletedCorrespondance = function() {
		var searchReferenceCourrier = 'NAN'; var searchReceptionDateStart = 'NAN'; var searchReceptionDateEnd = 'NAN'; var searchReferenceExpediteur = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchExpediteur = 'NAN'; var searchObjet = 'NAN';
	...
	$scope.saveConfirmCourrier = function() {
		...
		$http({
			url: context+"/correspondancedircab/rest/save",
			method: 'POST',
			data: formData,
			...
			if(data != "FAILED") {
				$scope.refreshList();
				Toastify({
					text: "La correspondance a été bien enregistrée sous la référence Nº "+data,
				...
				Toastify({
					text: "Impossible d'enregistrer la correspondance. Veuillez contacter l'administrateur du système.",
	...
	$scope.confirmRemoveCourrier = function() {
		...
		$http.post(context+$scope.baseUrl+"delete", angular.toJson($scope.wSubWorkId)).success(function(data, status) {
			...
			$scope.refreshList();
			Toastify({
				text: "La correspondance a été supprimée avec succès.",
				gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
			}).showToast();
		}).error(function(data, status, headers, config) {
			$scope.removeProgress = false;
			$('#removeCourrierModal').modal("hide");
			Toastify({
				text: "Impossible de supprimer la correspondance. Veuillez contacter l'administrateur du système.",
				gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
			}).showToast();
	});
	};
	...
	$scope.confirmSendCourrier = function() {
		...
		if($scope.dto.conseillerTechnique != null)
			formData.append("conseillerTechnique", $scope.dto.conseillerTechnique);
		if($scope.dto.transfereAu != null)
		...
		$http({
			url: context+"/correspondancedircab/rest/send",
			method: 'POST',
			data: formData,
			headers: {'Content-Type': undefined},
			transformRequest: angular.identity
			...
			if(data == "SUCCESS") {
				$scope.refreshList();
				Toastify({
					text: "La correspondance a été transférée avec succès.",
					gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
				}).showToast();
			} else {
				Toastify({
					text: "Impossible de transférer la correspondance. Veuillez contacter l'administrateur du système.",
					gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #e84e4e, #a33737)"}
				}).showToast();
			}
		}).error(function(data, status, headers, config) {
		...
		if($scope.cancelCourrierForm.$valid) {
			...
			$http({
				...
				if(data == "SUCCESS") {
					$scope.refreshList();
					Toastify({
						text: "La correspondance a été rejetée avec succès.",
						gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
					}).showToast();
				} else {
					Toastify({
						text: "Impossible de rejeter la correspondance. Veuillez contacter l'administrateur du système.",
						gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #e84e4e, #a33737)"}
					}).showToast();
				}
			}).error(function(data, status, headers, config) {
	...
	$scope.downloadFile = function() {
		exportExcelFile($scope.courrierTab, 'Correspondances.xlsx', 'Correspondances');
	}

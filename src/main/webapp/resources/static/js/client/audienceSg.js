app.controller('courrierController', function($scope, $http, $ngBootbox, $location, CRUDService, NotificationService, constants) {
	...
	$scope.initIndicators = function() {
		$http.get(context+$scope.baseUrl+"getCurrentCountByProfile").success(function(data, status) {   
			$scope.currentAudienceCountByProfile = data;
			$scope.taskByProfile = $scope.currentAudienceCountByProfile;
		});
	}
	...
	$scope.advancedSearchCurrentAudience = function() {
	...
	$scope.advancedSearchDoneAudience = function() {
		var searchReferenceCourrier = 'NAN'; var searchReceptionDateStart = 'NAN'; var searchReceptionDateEnd = 'NAN'; var searchReferenceExpediteur = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchExpediteur = 'NAN'; var searchObjet = 'NAN';
	...
	$scope.advancedSearchCompletedAudience = function() {
		var searchReferenceCourrier = 'NAN'; var searchReceptionDateStart = 'NAN'; var searchReceptionDateEnd = 'NAN'; var searchReferenceExpediteur = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchExpediteur = 'NAN'; var searchObjet = 'NAN';
	...
	$scope.saveConfirmCourrier = function() {
		...
		$http({
			url: context+"/audiencesg/rest/save",
			method: 'POST',
			data: formData,
			...
			if(data != "FAILED") {
				$scope.refreshList();
				Toastify({
					text: "La demande d'audience a été bien enregistrée sous la référence Nº "+data,
				...
				Toastify({
					text: "Impossible d'enregistrer la demande d'audience. Veuillez contacter l'administrateur du système.",
	...
	$scope.confirmRemoveCourrier = function() {
		...
		$http.post(context+$scope.baseUrl+"delete", angular.toJson($scope.wSubWorkId)).success(function(data, status) {
			...
			$scope.refreshList();
			Toastify({
				text: "La demande d'audience a été supprimée avec succès.",
				gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
			}).showToast();
		}).error(function(data, status, headers, config) {
			$scope.removeProgress = false;
			$('#removeCourrierModal').modal("hide");
			Toastify({
				text: "Impossible de supprimer la demande d'audience. Veuillez contacter l'administrateur du système.",
				gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
			}).showToast();
	});
	};
	...
	$scope.confirmSendCourrier = function() {
		...
		$http({
			url: context+"/audiencesg/rest/send",
			method: 'POST',
			data: formData,
			headers: {'Content-Type': undefined},
			transformRequest: angular.identity
			...
			if(data == "SUCCESS") {
				$scope.refreshList();
				Toastify({
					text: "La demande d'audience a été transférée avec succès.",
					gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
				}).showToast();
			} else {
				Toastify({
					text: "Impossible de transférer la demande d'audience. Veuillez contacter l'administrateur du système.",
					gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #e84e4e, #a33737)"}
				}).showToast();
			}
		}).error(function(data, status, headers, config) {
		...
		if($scope.cancelCourrierForm.$valid) {
			...
			$http({
				...
				if(data == "SUCCESS") {
					$scope.refreshList();
					Toastify({
						text: "La demande d'audience a été rejetée avec succès.",
						gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
					}).showToast();
				} else {
					Toastify({
						text: "Impossible de rejeter la demande d'audience. Veuillez contacter l'administrateur du système.",
						gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #e84e4e, #a33737)"}
					}).showToast();
				}
			}).error(function(data, status, headers, config) {
	...
	$scope.downloadFile = function() {
		exportExcelFile($scope.courrierTab, 'Audiences.xlsx', 'Audiences');
	}

app.controller('courrierController', function($scope, $http, $ngBootbox, $location, CRUDService, NotificationService, constants) {
	...
	$scope.baseUrl = "/titularisation/rest/";
	...
	$scope.initIndicators = function() {
		$http.get(context+$scope.baseUrl+"getCurrentCountByProfile").success(function(data, status) {   
			$scope.currentTitularisationCountByProfile = data;
			$scope.taskByProfile = $scope.currentTitularisationCountByProfile;
		});
	}
	...
	$scope.initListConseillers = function() {
		$http.get(context+$scope.profileBaseUrl+"getAllConseillers").success(function(data, status) {
			$scope.conseillers = data;
		});
	};
	...
	$scope.addCourrier = function() {
		...
		$http.get(context+$scope.senderBaseUrl+"getByName/"+profileName).success(function(data, status) {
			if(data != null) {
				$scope.dto.expediteur = data.nom;
				$scope.dto.dateReception = getCurrentDate();
				document.getElementById("dateReception").disabled = true;
				document.getElementById("expediteur").disabled = true;
				document.getElementById("dateReception").removeAttribute("required");
				document.getElementById("expediteur").removeAttribute("required");
				$scope.sender = true;
			}
		});
		if(profileName.includes("Conseiller")) {
			$scope.adviser = true;
		}
		$scope.dto.documents = [];
	...
	$scope.advancedSearchCurrentTitularisation = function() {
		var searchReferenceCourrier = 'NAN'; var searchReceptionDateStart = 'NAN'; var searchReceptionDateEnd = 'NAN'; var searchReferenceExpediteur = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchExpediteur = 'NAN'; var searchObjet = 'NAN';
		...
		if(!angular.isUndefined($scope.dto.search.receptionDateStartCurrent) && $scope.dto.search.receptionDateStartCurrent != '') {
			searchReceptionDateStart = $scope.dto.search.receptionDateStartCurrent.toLocaleString("default", { day: "2-digit" }) + "-" + $scope.dto.search.receptionDateStartCurrent.toLocaleString("default", { month: "2-digit" }) + "-" + $scope.dto.search.receptionDateStartCurrent.toLocaleString("default", { year: "numeric" });
		}
		if(!angular.isUndefined($scope.dto.search.receptionDateEndCurrent) && $scope.dto.search.receptionDateEndCurrent != '') {
			searchReceptionDateEnd = $scope.dto.search.receptionDateEndCurrent.toLocaleString("default", { day: "2-digit" }) + "-" + $scope.dto.search.receptionDateEndCurrent.toLocaleString("default", { month: "2-digit" }) + "-" + $scope.dto.search.receptionDateEndCurrent.toLocaleString("default", { year: "numeric" });
		}
		if(!angular.isUndefined($scope.dto.search.referenceExpediteurCurrent) && $scope.dto.search.referenceExpediteurCurrent != '') searchReferenceExpediteur = $scope.dto.search.referenceExpediteurCurrent.replaceAll("/", "-");
		...
		if($(".currentSender option:selected").text() != null && $(".currentSender option:selected").text() != '') searchExpediteur = $(".currentSender option:selected").text();
		...
		$scope.courrierCurrentTable.ajax.url($scope.baseUrl+'search/'+searchReferenceCourrier+'/'+searchReceptionDateStart+'/'+searchReceptionDateEnd+'/'+searchReferenceExpediteur+'/'+searchEnregistrementDateStart+'/'+searchEnregistrementDateEnd+'/'+searchExpediteur+'/'+searchObjet+'/current').load();
	};
	
	$scope.advancedSearchDoneTitularisation = function() {
		var searchReferenceCourrier = 'NAN'; var searchReceptionDateStart = 'NAN'; var searchReceptionDateEnd = 'NAN'; var searchReferenceExpediteur = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchExpediteur = 'NAN'; var searchObjet = 'NAN';
		...
		if(!angular.isUndefined($scope.dto.search.receptionDateStartDone) && $scope.dto.search.receptionDateStartDone != '') {
			searchReceptionDateStart = $scope.dto.search.receptionDateStartDone.toLocaleString("default", { day: "2-digit" }) + "-" + $scope.dto.search.receptionDateStartDone.toLocaleString("default", { month: "2-digit" }) + "-" + $scope.dto.search.receptionDateStartDone.toLocaleString("default", { year: "numeric" });
		}
		if(!angular.isUndefined($scope.dto.search.receptionDateEndDone) && $scope.dto.search.receptionDateEndDone != '') {
			searchReceptionDateEnd = $scope.dto.search.receptionDateEndDone.toLocaleString("default", { day: "2-digit" }) + "-" + $scope.dto.search.receptionDateEndDone.toLocaleString("default", { month: "2-digit" }) + "-" + $scope.dto.search.receptionDateEndDone.toLocaleString("default", { year: "numeric" });
		}
		if(!angular.isUndefined($scope.dto.search.referenceExpediteurDone) && $scope.dto.search.referenceExpediteurDone != '') searchReferenceExpediteur = $scope.dto.search.referenceExpediteurDone.replaceAll("/", "-");
		...
		if($(".doneSender option:selected").text() != null && $(".doneSender option:selected").text() != '') searchExpediteur = $(".doneSender option:selected").text();
		...
		$scope.courrierDoneTable.ajax.url($scope.baseUrl+'search/'+searchReferenceCourrier+'/'+searchReceptionDateStart+'/'+searchReceptionDateEnd+'/'+searchReferenceExpediteur+'/'+searchEnregistrementDateStart+'/'+searchEnregistrementDateEnd+'/'+searchExpediteur+'/'+searchObjet+'/done').load();
	};
	
	$scope.advancedSearchCompletedTitularisation = function() {
		var searchReferenceCourrier = 'NAN'; var searchReceptionDateStart = 'NAN'; var searchReceptionDateEnd = 'NAN'; var searchReferenceExpediteur = 'NAN'; var searchEnregistrementDateStart = 'NAN'; var searchEnregistrementDateEnd = 'NAN'; var searchExpediteur = 'NAN'; var searchObjet = 'NAN';
		...
		if(!angular.isUndefined($scope.dto.search.receptionDateStartCompleted) && $scope.dto.search.receptionDateStartCompleted != '') {
			searchReceptionDateStart = $scope.dto.search.receptionDateStartCompleted.toLocaleString("default", { day: "2-digit" }) + "-" + $scope.dto.search.receptionDateStartCompleted.toLocaleString("default", { month: "2-digit" }) + "-" + $scope.dto.search.receptionDateStartCompleted.toLocaleString("default", { year: "numeric" });
		}
		if(!angular.isUndefined($scope.dto.search.receptionDateEndCompleted) && $scope.dto.search.receptionDateEndCompleted != '') {
			searchReceptionDateEnd = $scope.dto.search.receptionDateEndCompleted.toLocaleString("default", { day: "2-digit" }) + "-" + $scope.dto.search.receptionDateEndCompleted.toLocaleString("default", { month: "2-digit" }) + "-" + $scope.dto.search.receptionDateEndCompleted.toLocaleString("default", { year: "numeric" });
		}
		if(!angular.isUndefined($scope.dto.search.referenceExpediteurCompleted) && $scope.dto.search.referenceExpediteurCompleted != '') searchReferenceExpediteur = $scope.dto.search.referenceExpediteurCompleted.replaceAll("/", "-");
		...
		if($(".completedSender option:selected").text() != null && $(".completedSender option:selected").text() != '') searchExpediteur = $(".completedSender option:selected").text();
		...
		$scope.courrierCloseTable.ajax.url($scope.baseUrl+'search/'+searchReferenceCourrier+'/'+searchReceptionDateStart+'/'+searchReceptionDateEnd+'/'+searchReferenceExpediteur+'/'+searchEnregistrementDateStart+'/'+searchEnregistrementDateEnd+'/'+searchExpediteur+'/'+searchObjet+'/completed').load();
	};
	...
	$scope.saveCourrier = function() {
		...
		if($scope.courrierForm.$valid) {
			if($scope.sender) {
				$scope.dto.dateReception = new Date();
			}
	...
	$scope.saveConfirmCourrier = function() {
		...
		formData.append("referenceExpediteur", $scope.dto.referenceExpediteur);
		formData.append("expediteur", $scope.dto.expediteur);
		formData.append("dateReception", $scope.dto.dateReception);
		...
		for(var doc of $scope.dto.documents) {
			//formData.append("documents[" + index + "]", $('input[type=file]')[index].files[0]);
			formData.append("documents", $('.attachment-save')[index].files[0]);
			index++;
		}
		$http({
			url: context+"/titularisation/rest/save",
			method: 'POST',
			data: formData,
			...
			if(data != "FAILED") {
				$scope.refreshList();
				Toastify({
					text: "Le projet de titularisation a été bien enregistré sous la référence Nº "+data,
				...
				Toastify({
					text: "Impossible d'enregistrer le projet de titularisation. Veuillez contacter l'administrateur du système.",
	...
	$scope.confirmRemoveCourrier = function() {
		...
		$http.post(context+$scope.baseUrl+"delete", angular.toJson($scope.wSubWorkId)).success(function(data, status) {
			...
			$scope.refreshList();
			Toastify({
				text: "Le projet de titularisation a été supprimé avec succès.",
				gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
			}).showToast();
		}).error(function(data, status, headers, config) {
			$scope.removeProgress = false;
			$('#removeCourrierModal').modal("hide");
			Toastify({
				text: "Impossible de supprimer le projet de titularisation. Veuillez contacter l'administrateur du système.",
				gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
			}).showToast();
	});
	};
	...
	$scope.confirmSendCourrier = function() {
		...
		formData.append("conseillerTechnique", $scope.dto.conseillerTechnique);
		formData.append("transfereAu", $scope.dto.transfereAu);
		var index = 0;
		for(var doc of $scope.dto.documents) {
		...
		$http({
			url: context+"/titularisation/rest/send",
			method: 'POST',
			data: formData,
			headers: {'Content-Type': undefined},
			transformRequest: angular.identity
			...
			if(data == "SUCCESS") {
				$scope.refreshList();
				Toastify({
					text: "Le projet de titularisation a été transféré avec succès.",
					gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
				}).showToast();
			} else {
				Toastify({
					text: "Impossible de transférer le projet de titularisation. Veuillez contacter l'administrateur du système.",
					gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #e84e4e, #a33737)"}
				}).showToast();
			}
		}).error(function(data, status, headers, config) {
		...
		if($scope.cancelCourrierForm.$valid) {
			...
			$http({
				...
				if(data == "SUCCESS") {
					$scope.refreshList();
					Toastify({
						text: "Le projet de titularisation a été rejeté avec succès.",
						gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #4fea98, #38a56b)"}
					}).showToast();
				} else {
					Toastify({
						text: "Impossible de rejeter le projet de titularisation. Veuillez contacter l'administrateur du système.",
						gravity: "top", position: "right", duration: 4000, close: true, stopOnFocus: true, style: {fontWeight: "bold", background: "linear-gradient(to right, #e84e4e, #a33737)"}
					}).showToast();
				}
			}).error(function(data, status, headers, config) {
	...
	$scope.exportCourriers = function(name) {
		...
		$scope.courrierTab = [];
		var title = ["REFERENCE COURRIER", "DATE RECEPTION", "DATE ENREGISTREMENT", "REFERENCE EXPEDITEUR", "EXPEDITEUR", "OBJET", "TRANSFERE A", "STATUT"];
		...
		for(var i = 0, l = inputs.length; i < l; ++i) {
			if(inputs[i].checked) {
				$http.get(context+$scope.baseUrl+"loadByReference/"+inputs[i].value.replaceAll("/", "-")).success(function(data, status) {
					...
					courrier[1] = data.dateReception;
					courrier[2] = data.dateEnregistrement;
					courrier[3] = data.referenceExpediteur;
					courrier[4] = data.expediteur;
					courrier[5] = data.objet;
					courrier[6] = data.taskTitle;
					var status = "";
	...
	$scope.downloadFile = function() {
		exportExcelFile($scope.courrierTab, 'Projets de titularisation.xlsx', 'Projets de titularisation');
	}

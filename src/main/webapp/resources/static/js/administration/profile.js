app.controller('profileController', function($scope, $http, $ngBootbox, $location, CRUDService, NotificationService, constants) {
	...
	$scope.initListOTGroups = function() {
		$http.get(context+$scope.performerBaseUrl+"getAllGroups").success(function(data, status) {
			$scope.groups = data;
		});
	};
	...
	$scope.saveProfile = function() {
		if($scope.profileForm.$valid) {
			$http.get(context+$scope.baseUrl+"loadByPerformer/"+$scope.dto.performerId).success(function(data, status) {
				if(data == null) {
...
//	};
	...
	resetUser = function(id) {
		if(confirm("Voulez-vous réinitialiser le mot de passe de ce compte ?") == true) {
			$http.get(context+$scope.userBaseUrl+"load/"+id).success(function(data, status) {
...
//	};
	...
	disableUser = function(id) {
		if(confirm("Voulez-vous vraiment désactiver cet utilisateur ?") == true) {
	...
	$scope.init = function(){
		var columns = [ 
			{mDataProp: 'id', "mRender": function(data, type, full) {
				return '<div class="form-check"><input class="form-check-input performerId" type="checkbox" value="'+full.performerName+'" onclick="check(\'performerId\')" /></div>';
			}},
			{mDataProp: 'performerName'},
			{mDataProp: 'id', "mRender": function(data, type, full) {
				return '<a href="#" onclick="editProfile();" class="edit-item-btn"><i style="font-size: 17px" class="ri-pencil-fill align-bottom me-2"></i></a> <a href="#" data-bs-toggle="modal" data-bs-target="#deleteProfileModal" class="remove-item-btn"><i style="font-size: 17px" class="ri-delete-bin-fill align-bottom me-2 text-danger"></i></a>';
			}}
		];

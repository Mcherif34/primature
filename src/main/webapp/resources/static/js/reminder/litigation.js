app.controller('litigationController', function($scope, $http, $ngBootbox, $location, CRUDService, NotificationService, constants) {
	$scope.dto = {};
	$scope.filtre = {};
	$scope.litigationCurrentTable = null;
	$scope.litigationCloseTable = null;
	...
	$scope.litigationStageRequiredError = false;
	$scope.litigationStageTechnicalError = false;
	$scope.litigationStageSuccess = false;
	$scope.litigationStageDateRequiredError = false;
	$scope.litigationStageDateTechnicalError = false;
	$scope.litigationStageDateSuccess = false;
	$scope.notifications = null;
	$scope.notificationsCount = 0;
	
	$scope.baseUrl = "/reminder/litigation/rest/";
	...
	$scope.addLitigation = function() {
		...
		$scope.dto.litigationStatusId = 1;
		$scope.editLitigationForm = false;
	...
	$scope.addLitigationStage = function() {
		$scope.dto.litigationStage = {};
		$scope.editLitigationStageForm = false;
		$scope.addLitigationStageForm = true;
		$scope.litigationStageSuccess = false;
		$scope.litigationStageTechnicalError = false;
		$scope.litigationStageRequiredError = false;
		$('#litigationStageModal').modal("show");
	};
	...
	$scope.addLitigationInvoice = function() {
		...
		if(!angular.isUndefined($scope.invoiceDetail)) {
			...
			$http.get(context+"/credit/invoice/rest/load/"+$scope.invoiceDetail.id).success(function(data, status) {
				...
				$scope.litigationTechnicalError = false;
				$scope.litigationRequiredError = false;
				$('#litigationInvoiceModal').modal("show");
			});
		}
	};
	...
	editLitigationStage = function(id) {
		$http.get(context+$scope.litigationStageBaseUrl+"load/"+id).success(function(data, status) { 
			$scope.litigationStage = data;
			$scope.dto.litigationStage = $scope.litigationStage;
			CRUDService.edit($scope);
			$scope.addLitigationStageForm = false;
			$scope.editLitigationStageForm = true;
			$scope.litigationStageSuccess = false;
			$scope.litigationStageTechnicalError = false;
			$scope.litigationStageRequiredError = false;
			$('#litigationStageModal').modal("show");
	...
	$scope.editLitigationStageDate = function() {
		CRUDService.edit($scope);
		$scope.litigationStageDateSuccess = false;
		$scope.litigationStageDateTechnicalError = false;
		$scope.litigationStageDateRequiredError = false;
		$('#litigationStageDateModal').modal("show");
	};
	...
	showClientDetail = function(id) {
		$http.get(context+$scope.clientBaseUrl+"load/"+id).success(function(data, status) { 
			...
			$scope.litigationCurrentClientTable.ajax.url($scope.baseUrl+'list/current/'+id).load();
	...
	$scope.loadLitigationStage = function(id) {
		$http.get(context+$scope.litigationStageBaseUrl+"load/"+id).success(function(data, status) { 
			$scope.litigationStage = data;
			$scope.dto.litigationStage = $scope.litigationStage;
		});
		...
		if($scope.litigationForm.$valid) {
			if($scope.dto.code == null) {
				...
				$http.get(context+"/reminder/litigation/rest/getAll").success(function(data, status) {   
					...
					$scope.dto.code = "LT" + currentDate.getFullYear() + (currentDate.getMonth()+1).toString().padStart(2, '0') + seqNum.toString().padStart(3, '0');
	...
	$scope.saveLitigationInvoice = function() {
		if($scope.litigationForm.$valid) {
			if($scope.dto.litigation.code == null) {
				...
				$http.get(context+"/reminder/litigation/rest/getAll").success(function(data, status) {   
					...
					$http.post(context+$scope.baseUrl+"save", angular.toJson($scope.dto.litigation)).success(function(data, status) {
	...
	$scope.closeLitigation = function() {
		if(confirm("Voulez-vous vraiment clôturer ce dossier de litige ?") == true) {
			$scope.dto.litigationStatusId = 2;
			CRUDService.save($scope,$scope.dto).success(function(data, status) {   
				CRUDService.setEntityLoaded($scope,data);
				$scope.refreshList();
			}).error(function(data, status, headers, config) {
	...
	$scope.saveLitigationStage = function() {
		if($scope.litigationStageForm.$valid) {
			$scope.dto.litigationStage.litigationId = $scope.dto.id;
			$http.post( context+$scope.litigationStageBaseUrl+"save", angular.toJson($scope.dto.litigationStage)).success(function(data, status) {   
				$scope.dto = {};
				$scope.load($scope.selected.id);
				$scope.litigationStageTechnicalError = false;
				$scope.litigationStageRequiredError = false;
				$scope.litigationStageSuccess = true;
				$('#litigationStageModal').modal("hide");
			}).error(function(data, status, headers, config) {
				$scope.litigationStageTechnicalError = true;
			});
		} else {
			$scope.litigationStageRequiredError = true;
		}
	};
	
	$scope.saveLitigationStageDate = function() {
		if($scope.litigationStageDateForm.$valid) {
			$http.post( context+$scope.litigationStageBaseUrl+"save", angular.toJson($scope.dto.litigationStage)).success(function(data, status) {   
				$scope.dto = {};
				$scope.load($scope.selected.id);
				$scope.litigationStageDateTechnicalError = false;
				$scope.litigationStageDateRequiredError = false;
				$scope.litigationStageDateSuccess = true;
				$('#litigationStageDateModal').modal("hide");
			}).error(function(data, status, headers, config) {
				$scope.litigationStageDateTechnicalError = true;
			});
		} else {
			$scope.litigationStageDateRequiredError = true;
		}
	};
	...
	removeLitigationStage = function(id) {
		$scope.dto = {};
		if(confirm("Voulez-vous vraiment supprimer cet enregistrement ?") == true) {
			$scope.confirmDeleteLitigationStage(id);
		};
	};

	$scope.confirmDeleteLitigationStage = function(id) {
		$http.post( context+$scope.litigationStageBaseUrl+"delete/"+id).success(function(data, status) { 
			$scope.dto = {};
			$scope.load($scope.selected.id);
		}).error(function(data, status, headers, config) {
			console.log("error");
	...
	$scope.init = function() {
		var columns = [ 
			...
			{mDataProp: 'id', "mRender": function(data, type, full) {
				result = '<a href="#" onclick="editLitigation('+full.id+');" class="btn btn-fw orange" style="min-width: 1rem; padding-left: 8px; padding-right: 8px; padding-top: 3px; padding-bottom: 3px;"><i class="fa fa-pencil"></i></a> <a href="#" onclick="removeLitigation('+full.id+');" class="btn btn-fw danger" style="min-width: 1rem; padding-left: 8px; padding-right: 8px; padding-top: 3px; padding-bottom: 3px;"><i class="fa fa-trash"></i></a>';
				return result;
			}}
		];
		$scope.$parent.litigationCurrentTable = TableManager.init("litigationCurrentTable", $scope.$parent.baseUrl+"list/current", columns);
...
app.controller('litigationCloseTableController', function($scope,$http) {
	$scope.$watch('$parent.dto.id', function(newValue, oldValue) {
		if(!angular.isUndefined(newValue) && newValue != null) {
			$scope.$parent.litigationStageTable.ajax.url('/reminder/litigationStage/rest/list/'+newValue).load();
		}
	...
	$scope.init = function() {
		...
		$scope.$parent.litigationCloseTable = TableManager.init("litigationCloseTable", $scope.$parent.baseUrl+"list/close", columns);
		
		$scope.$parent.litigationCloseTable.on('select', function (e, dt, type, indexes) {
			var rowData = $scope.$parent.litigationCloseTable.rows(indexes).data().toArray();
			var id = null;
			if(rowData.length > 0) {
				$scope.$parent.selected = rowData[0];
				id = rowData[0].id;
...
app.controller('litigationStageTableController', function($scope,$http) {
	$scope.init = function() {
		var columns = [
			...
			{mDataProp: 'id', "mRender": function(data, type, full) {
				result = '<a href="#" onclick="editLitigationStage('+full.id+');" class="btn btn-fw orange" style="min-width: 1rem; padding-left: 8px; padding-right: 8px; padding-top: 3px; padding-bottom: 3px;"><i class="fa fa-pencil"></i></a> <a href="#" onclick="removeLitigationStage('+full.id+');" class="btn btn-fw danger" style="min-width: 1rem; padding-left: 8px; padding-right: 8px; padding-top: 3px; padding-bottom: 3px;"><i class="fa fa-trash"></i></a>';
				return result;
			}}
		];
		...
		$scope.$parent.litigationStageTable.on('select', function (e, dt, type, indexes) {
			var rowData = $scope.$parent.litigationStageTable.rows(indexes).data().toArray();
			var id = null;
			if(rowData.length > 0) {
				id = rowData[0].id;
				$scope.$parent.loadLitigationStage(id);
			}
		});
	};
});
...
app.controller('litigationCurrentClientTableController', function($scope,$http) {
	$scope.init = function() {
		...
		$scope.$parent.litigationCurrentClientTable = TableManager.init("litigationCurrentClientTable", $scope.$parent.baseUrl+"list/current/-1", columns);
	};
});

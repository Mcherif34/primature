app.controller('scoringController', function($scope, $http, $ngBootbox, $location, CRUDService, NotificationService, constants) {
	$scope.dto = {};
	$scope.filtre = {};
	$scope.scoringTable = null;
	$scope.legalScoreTable = null;
	$scope.moralityScoreTable = null;
	$scope.financialScoreTable = null;
	...
	$scope.mode = null;
	$scope.hidden = false;
	$scope.legalScoreRequiredError = false;
	$scope.legalScoreTechnicalError = false;
	$scope.legalScoreSuccess = false;
	$scope.moralityScoreRequiredError = false;
	$scope.moralityScoreTechnicalError = false;
	$scope.moralityScoreSuccess = false;
	$scope.financialScoreRequiredError = false;
	$scope.financialScoreTechnicalError = false;
	$scope.financialScoreSuccess = false;
	...
	$scope.baseUrl = "/credit/scoring/rest/";
	$scope.legalScoreBaseUrl = "/credit/scoring/legalScore/rest/";
	$scope.moralityScoreBaseUrl = "/credit/scoring/moralityScore/rest/";
	$scope.financialScoreBaseUrl = "/credit/scoring/financialScore/rest/";
	...
	$scope.init = function() {
		...
		$scope.initListMoralityTypes();
		$scope.initListJobStatus();
	...
	$scope.initListMoralityTypes = function() {
		$http.get(context+"/administration/referentiel/moralityType/rest/getAll").success(function(data, status) {   
			$scope.moralityTypes = data;
		});
	};
	
	$scope.initListJobStatus = function() {
		$http.get(context+"/administration/referentiel/jobStatus/rest/getAll").success(function(data, status) {   
	...
	$scope.addLegalScore = function() {
		$scope.dto.legalScore = {};
		$scope.dto.legalScore.creationDate = $scope.dto.creationDate;
		$scope.dto.legalScore.legalFormId = $scope.dto.legalFormId;
		$scope.dto.legalScore.governanceId = $scope.dto.governanceId;
		$scope.editLegalScoreForm = false;
		$scope.addLegalScoreForm = true;
		$scope.legalScoreSuccess = false;
		$scope.legalScoreTechnicalError = false;
		$scope.legalScoreRequiredError = false;
		$('#legalScoreModal').modal("show");
	};
	
	$scope.addMoralityScore = function() {
		$scope.dto.moralityScore = {};
		$scope.dto.moralityScore.creationDate = $scope.dto.creationDate;
		$scope.editMoralityScoreForm = false;
		$scope.addMoralityScoreForm = true;
		$scope.moralityScoreSuccess = false;
		$scope.moralityScoreTechnicalError = false;
		$scope.moralityScoreRequiredError = false;
		$('#moralityScoreModal').modal("show");
	};
	
	$scope.addFinancialScore = function() {
		$scope.dto.financialScore = {};
		$scope.dto.financialScore.year = new Date().getFullYear();
		$scope.editFinancialScoreForm = false;
		$scope.addFinancialScoreForm = true;
		$scope.financialScoreSuccess = false;
		$scope.financialScoreTechnicalError = false;
		$scope.financialScoreRequiredError = false;
		$('#financialScoreModal').modal("show");
	};
	...
	$scope.setLegalAge = function() {
		var date = $scope.dto.legalScore.creationDate.split('/');
		if($scope.dto.legalScore.year > date[2])
			$scope.dto.legalScore.age = $scope.dto.legalScore.year - date[2];
		else
			$scope.dto.legalScore.age = 0;
	}
	
	$scope.setMoralityAge = function() {
		var date = $scope.dto.moralityScore.creationDate.split('/');
		if($scope.dto.moralityScore.year > date[2])
			$scope.dto.moralityScore.age = $scope.dto.moralityScore.year - date[2];
		else
			$scope.dto.moralityScore.age = 0;
	}
	...
	editLegalScore = function(id) {
		$http.get(context+$scope.legalScoreBaseUrl+"load/"+id).success(function(data, status) { 
			$scope.legalScore = data;
			$scope.dto.legalScore = $scope.legalScore;
			CRUDService.edit($scope);
			$scope.addLegalScoreForm = false;
			$scope.editLegalScoreForm = true;
			$scope.legalScoreSuccess = false;
			$scope.legalScoreTechnicalError = false;
			$scope.legalScoreRequiredError = false;
			$('#legalScoreModal').modal("show");
	...
	editMoralityScore = function(id) {
		$http.get(context+$scope.moralityScoreBaseUrl+"load/"+id).success(function(data, status) { 
			$scope.moralityScore = data;
			$scope.dto.moralityScore = $scope.moralityScore;
			CRUDService.edit($scope);
			$scope.addMoralityScoreForm = false;
			$scope.editMoralityScoreForm = true;
			$scope.moralityScoreSuccess = false;
			$scope.moralityScoreTechnicalError = false;
			$scope.moralityScoreRequiredError = false;
			$('#moralityScoreModal').modal("show");
	...
	editFinancialScore = function(id) {
		$http.get(context+$scope.financialScoreBaseUrl+"load/"+id).success(function(data, status) { 
			$scope.financialScore = data;
			$scope.dto.financialScore = $scope.financialScore;
			CRUDService.edit($scope);
			$scope.addFinancialScoreForm = false;
			$scope.editFinancialScoreForm = true;
			$scope.financialScoreSuccess = false;
			$scope.financialScoreTechnicalError = false;
			$scope.financialScoreRequiredError = false;
			$('#financialScoreModal').modal("show");
	...
	$scope.saveLegalScore = function() {
		if($scope.legalScoreForm.$valid) {
			$scope.dto.legalScore.clientId = $scope.dto.id;
			$http.post(context+$scope.legalScoreBaseUrl+"save", angular.toJson($scope.dto.legalScore)).success(function(data, status) {
				$scope.dto = {};
				$scope.load($scope.selected.id);
				$scope.legalScoreTechnicalError = false;
				$scope.legalScoreRequiredError = false;
				$scope.legalScoreSuccess = true;
				$('#legalScoreModal').modal("hide");
				$scope.scoringTable.ajax.reload();
			}).error(function(data, status, headers, config) {
				$scope.legalScoreTechnicalError = true;
			});
		} else {
			$scope.legalScoreRequiredError = true;
		}
	};
	
	$scope.saveMoralityScore = function() {
		if($scope.moralityScoreForm.$valid) {
			$scope.dto.moralityScore.clientId = $scope.dto.id;
			$http.post(context+$scope.moralityScoreBaseUrl+"save", angular.toJson($scope.dto.moralityScore)).success(function(data, status) {
				$scope.dto = {};
				$scope.load($scope.selected.id);
				$scope.moralityScoreTechnicalError = false;
				$scope.moralityScoreRequiredError = false;
				$scope.moralityScoreSuccess = true;
				$('#moralityScoreModal').modal("hide");
				$scope.scoringTable.ajax.reload();
			}).error(function(data, status, headers, config) {
				$scope.moralityScoreTechnicalError = true;
			});
		} else {
			$scope.moralityScoreRequiredError = true;
		}
	};
	
	$scope.saveFinancialScore = function() {
		if($scope.financialScoreForm.$valid) {
			$scope.dto.financialScore.clientId = $scope.dto.id;
			$http.post(context+$scope.financialScoreBaseUrl+"save", angular.toJson($scope.dto.financialScore)).success(function(data, status) {
				$scope.dto = {};
				$scope.load($scope.selected.id);
				$scope.financialScoreTechnicalError = false;
				$scope.financialScoreRequiredError = false;
				$scope.financialScoreSuccess = true;
				$('#financialScoreModal').modal("hide");
				$scope.scoringTable.ajax.reload();
			}).error(function(data, status, headers, config) {
				$scope.financialScoreTechnicalError = true;
			});
		} else {
			$scope.financialScoreRequiredError = true;
		}
	};
	...
	$scope.refreshList = function() {
		$scope.legalScoreTable.ajax.reload();
		$scope.moralityScoreTable.ajax.reload();
		$scope.financialScoreTable.ajax.reload();
		$scope.scoringTable.ajax.reload();
		$scope.dto = {};
	};
	
	removeLegalScore = function(id) {
		$scope.dto = {};
		if(confirm("Voulez-vous vraiment supprimer cet enregistrement ?") == true) {
			$scope.confirmDeleteLegalScore(id);
		};
	};

	$scope.confirmDeleteLegalScore = function(id) {
		$http.post(context+$scope.legalScoreBaseUrl+"delete/"+id).success(function(data, status) { 
			$scope.dto = {};
			$scope.load($scope.selected.id);
		}).error(function(data, status, headers, config) {
			console.log("error");
	...
	removeMoralityScore = function(id) {
		$scope.dto = {};
		if(confirm("Voulez-vous vraiment supprimer cet enregistrement ?") == true) {
			$scope.confirmDeleteMoralityScore(id);
		};
	};

	$scope.confirmDeleteMoralityScore = function(id) {
		$http.post(context+$scope.moralityScoreBaseUrl+"delete/"+id).success(function(data, status) { 
			$scope.dto = {};
			$scope.load($scope.selected.id);
		}).error(function(data, status, headers, config) {
			console.log("error");
	...
	removeFinancialScore = function(id) {
		$scope.dto = {};
		if(confirm("Voulez-vous vraiment supprimer cet enregistrement ?") == true) {
			$scope.confirmDeleteFinancialScore(id);
		};
	};

	$scope.confirmDeleteFinancialScore = function(id) {
		$http.post(context+$scope.financialScoreBaseUrl+"delete/"+id).success(function(data, status) { 
			$scope.dto = {};
			$scope.load($scope.selected.id);
		}).error(function(data, status, headers, config) {
			console.log("error");
...
app.controller('scoringTableController', function($scope,$http) {
	$scope.$watch('$parent.dto.id', function(newValue, oldValue) {
		if(!angular.isUndefined(newValue) && newValue != null) {
			$scope.$parent.legalScoreTable.ajax.url('/credit/scoring/legalScore/rest/list/'+newValue).load();
			$scope.$parent.moralityScoreTable.ajax.url('/credit/scoring/moralityScore/rest/list/'+newValue).load();
			$scope.$parent.financialScoreTable.ajax.url('/credit/scoring/financialScore/rest/list/'+newValue).load();
		}
		else {
			$scope.$parent.legalScoreTable.ajax.url('/credit/scoring/legalScore/rest/list/-1').load();
			$scope.$parent.moralityScoreTable.ajax.url('/credit/scoring/moralityScore/rest/list/-1').load();
			$scope.$parent.financialScoreTable.ajax.url('/credit/scoring/financialScore/rest/list/-1').load();
		}
	});
	$scope.init = function() {
		var columns = [
			...
			{mDataProp: 'id', "mRender": function(data, type, full) {
				var result = Math.round((full.financialScore*1.5 + full.legalScore + full.moralityScore)/6 * 100)/100;
				if(result == 0)
					return '<span class="label">N/A</span>';
				return result;
			}},
			{mDataProp: 'financialScore', "mRender": function(data, type, full) {
				var result = full.financialScore;
				if(result == 0)
					return '<span class="label">N/A</span>';
				return result;
			}},
			{mDataProp: 'legalScore', "mRender": function(data, type, full) {
				var result = full.legalScore;
				if(result == 0)
					return '<span class="label">N/A</span>';
				return result;
			}},
			{mDataProp: 'moralityScore', "mRender": function(data, type, full) {
				var result = full.moralityScore;
				if(result == 0)
					return '<span class="label">N/A</span>';
				return result;
			}},
		...
		$scope.$parent.scoringTable = TableManager.init("scoringTable", $scope.$parent.baseUrl+"list/"+new Date().getFullYear(), columns);
		
		$scope.$parent.scoringTable.on('select', function (e, dt, type, indexes) {
			var rowData = 	$scope.$parent.scoringTable.rows(indexes).data().toArray();
			var id = null;
			if(rowData.length > 0) {
...
app.controller('legalScoreTableController', function($scope,$http) {
	$scope.init = function() {
		var columns = [
			{mDataProp: 'id', "visible": false},
			{mDataProp: 'year'},
			{mDataProp: 'year', "mRender": function(data, type, full) {
				var result = '';
				if(full.age == 1) {
					result = full.age+' an';
				}
			...
			{mDataProp: 'legalFormName'},
			{mDataProp: 'governanceName'},
			{mDataProp: 'id', "mRender": function(data, type, full) {
				result = '<a href="#" onclick="editLegalScore('+full.id+');" class="btn btn-fw info" style="min-width: 1rem; padding-left: 8px; padding-right: 8px; padding-top: 3px; padding-bottom: 3px;"><i class="fa fa-pencil"></i></a> <a href="#" onclick="removeLegalScore('+full.id+');" class="btn btn-fw danger" style="min-width: 1rem; padding-left: 8px; padding-right: 8px; padding-top: 3px; padding-bottom: 3px;"><i class="fa fa-trash"></i></a>';
				return result;
			}}
		];
		$scope.$parent.legalScoreTable = TableManager.init("legalScoreTable", $scope.$parent.legalScoreBaseUrl+"list/-1", columns);
	};
});

app.controller('moralityScoreTableController', function($scope,$http) {
	$scope.init = function() {
		var columns = [
			{mDataProp: 'id', "visible": false},
			{mDataProp: 'year'},
			...
			{mDataProp: 'moralityTypeName'},
			{mDataProp: 'jobStatusName'},
			{mDataProp: 'id', "mRender": function(data, type, full) {
				result = '<a href="#" onclick="editMoralityScore('+full.id+');" class="btn btn-fw info" style="min-width: 1rem; padding-left: 8px; padding-right: 8px; padding-top: 3px; padding-bottom: 3px;"><i class="fa fa-pencil"></i></a> <a href="#" onclick="removeMoralityScore('+full.id+');" class="btn btn-fw danger" style="min-width: 1rem; padding-left: 8px; padding-right: 8px; padding-top: 3px; padding-bottom: 3px;"><i class="fa fa-trash"></i></a>';
				return result;
			}}
		];
		$scope.$parent.moralityScoreTable = TableManager.init("moralityScoreTable", $scope.$parent.moralityScoreBaseUrl+"list/-1", columns);
	};
});

app.controller('financialScoreTableController', function($scope,$http) {
	$scope.init = function() {
		var columns = [
			{mDataProp: 'id', "visible": false},
			{mDataProp: 'year'},
			{mDataProp: 'ca', "mRender": function(data, type, full) {
				if(full.ca != null)
					return new Intl.NumberFormat('fr-FR', {minimumFractionDigits: 2, maximumFractionDigits: 2}).format(full.ca);
				return 0;
			}},
			{mDataProp: 'capitals', "mRender": function(data, type, full) {
				if(full.capitals != null)
					return new Intl.NumberFormat('fr-FR', {minimumFractionDigits: 2, maximumFractionDigits: 2}).format(full.capitals);
				return 0;
			}},
			{mDataProp: 'equity', "mRender": function(data, type, full) {
				if(full.equity != null)
					return new Intl.NumberFormat('fr-FR', {minimumFractionDigits: 2, maximumFractionDigits: 2}).format(full.equity);
				return 0;
			}},
			{mDataProp: 'debts', "mRender": function(data, type, full) {
				if(full.debts != null)
					return new Intl.NumberFormat('fr-FR', {minimumFractionDigits: 2, maximumFractionDigits: 2}).format(full.debts);
				return 0;
			}},
			{mDataProp: 'netProfit', "mRender": function(data, type, full) {
				if(full.netProfit != null)
					return new Intl.NumberFormat('fr-FR', {minimumFractionDigits: 2, maximumFractionDigits: 2}).format(full.netProfit);
				return 0;
			}},
			{mDataProp: 'id', "mRender": function(data, type, full) {
				result = '<a href="#" onclick="editFinancialScore('+full.id+');" class="btn btn-fw info" style="min-width: 1rem; padding-left: 8px; padding-right: 8px; padding-top: 3px; padding-bottom: 3px;"><i class="fa fa-pencil"></i></a> <a href="#" onclick="removeFinancialScore('+full.id+');" class="btn btn-fw danger" style="min-width: 1rem; padding-left: 8px; padding-right: 8px; padding-top: 3px; padding-bottom: 3px;"><i class="fa fa-trash"></i></a>';
				return result;
			}}
		];
		$scope.$parent.financialScoreTable = TableManager.init("financialScoreTable", $scope.$parent.financialScoreBaseUrl+"list/-1", columns);
	};
});

app.controller('investisseurController', function($scope, $http, $ngBootbox, $location, CRUDService, NotificationService, constants) {
	...
	$scope.baseUrl = "/referentiel/investisseur/rest/";
	...
	$scope.initListNatures = function() {
		$http.get(context+"/list/REF/investisseur/nature").success(function(data, status) {   
			$scope.natures = data;
		});
	};
...
app.controller('investisseurTableController', function($scope,$http) {
	$scope.init = function() {
		var columns = [ 
		...
		$scope.$parent.table = TableManager.init("investisseurTable", $scope.$parent.baseUrl+"list", columns);
